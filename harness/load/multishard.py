    ...
    *,
    ...
    for k in range(engines):
        env = _node_env(
            ...
            name_prefix=f"E{k}",  # DISJOINT connection names ⇒ disjoint FIFO lanes on the shared store
        )
        env = _apply_cluster_env(env, cluster_enabled=cluster_enabled, node_id=f"ms-e{k}")
        nodes.append(
            EngineNode(
                f"ms-e{k}",
                api_ports[k],
    ...
    try:
        ...
        per_engine = await _attribute_engines(nodes)
...
async def _attribute_engines(nodes: list[EngineNode]) -> tuple[EngineAttribution, ...]:
    """Read each engine's OWN ``/connections`` (off the event loop) and attribute its inbound rows.

    On the shared store each engine k names its lanes ``IB_E{k}_CS_...`` (via MEFOR_CONNSCALE_NAME_
    PREFIX). A row this engine reports whose name does NOT carry its own ``E{k}_`` tag would mean a
    cross-engine steal (a same-named lane shared across engines) — the disjoint-isolation the gate
    requires would be broken. We read per engine (not the aggregate poller) precisely to catch that."""
    loop = asyncio.get_running_loop()
    return tuple(await loop.run_in_executor(None, _attribute_engines_sync, nodes))


def _attribute_engines_sync(nodes: list[EngineNode]) -> list[EngineAttribution]:
    from messagefoundry.console.client import ApiError, EngineClient
