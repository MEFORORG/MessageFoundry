...
* ``MEFOR_SHARDCERT_SHARDS``       — comma list of shard ids (default ``a,b,c,d``). Shard ``i`` (sorted)
  owns inbound ``IB_S_<id>`` on ``inbound_base + i``.
* ``MEFOR_SHARDCERT_INBOUND_BASE`` — first inbound MLLP port; shard ``i`` binds ``base + i`` (default 3600).
...
class ShardCertShape:
    ...
    def inbound_port(self, shard_index: int) -> int:
        """Shard ``i`` (in sorted order) binds ``inbound_base + i`` — contiguous so the driver can map
        one persistent connection per shard as ``base + i``."""
        return self.inbound_base + shard_index
...
def fifo_lane(shard: str, dest_index: int) -> str:
    """The per-(source-shard, destination) FIFO lane key stamped into MSH-6.
