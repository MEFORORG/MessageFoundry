...
def leak_check(text: str) -> list[str]:
    """Forbidden-token hits in ``text`` (empty = clean) — the tee's fail-closed leak gate. The site
    code is checked **field-anchored** (matching the replace path), so a scrub miss is caught without
    false-positiving on a value that merely contains a site-code run."""
    hits = scan_text(text, include_estate=True)
    if message_has_site_code(text):
        hits.append("site-code pattern")
    return hits
