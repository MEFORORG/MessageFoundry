...
# --- connection controls: a reload / DR broadcast must NOT resume a paused outbound (PR3) ----------
...
async def test_reload_and_dr_notify_do_not_resume_paused_outbound(
    store: MessageStore, tmp_path: Path, claim_mode: str
) -> None:
    """A /config/reload (which broadcasts notify_work to every dispatcher) AND a DR/replay notify_work
    must NOT resurrect a deliberately operator-PAUSED outbound (the reload-surviving _outbound_paused is
    the source of truth). The paused lane retains its queued rows PENDING, delivers ZERO while paused,
    and keeps busy_violations==0; start_outbound then resumes a clean FIFO drain."""
    reg = Registry()
    reg.add_outbound(
        OutboundConnection("out", ConnectionSpec(ConnectorType.FILE, {"directory": str(tmp_path)}))
    )
    ...
    try:
        ...
        elapsed = 0.0
        while runner.outbound_status("out") != "stopped":
            await asyncio.sleep(0.01)
            elapsed += 0.01
            assert elapsed < 3.0, "outbound did not quiesce to 'stopped'"

        # Queue rows behind the pause.
        for i in range(4):
            await store.enqueue_message(
                channel_id="c", raw=ADT, deliveries=[("out", f"p{i}")], source_type="file"
            )

        # A whole-config reload (same outbound) broadcasts notify_work to every dispatcher — it must NOT
        # resume the paused lane (the connector is kept warm across the reload too).
        same = Registry()
        same.add_outbound(
            OutboundConnection(
                "out", ConnectionSpec(ConnectorType.FILE, {"directory": str(tmp_path)})
            )
        )
        await runner.reload(same)
        assert runner._destinations["out"] is rec  # unchanged spec -> connector kept warm
        # A DR/replay notify_work must NOT resume it either.
        runner.notify_work()
        await asyncio.sleep(0.2)

        assert runner.outbound_running("out") is False  # still paused across reload + notify_work
        assert len(rec.delivered) == 0  # ZERO rows delivered while paused
        counts = await _outbound_status_counts(store, "out")
        assert counts.get(OutboxStatus.PENDING.value, 0) == 4  # retained PENDING
        assert counts.get(OutboxStatus.INFLIGHT.value, 0) == 0  # nothing stranded/claimed
        if claim_mode == "pooled":
            assert runner._dispatchers[Stage.OUTBOUND].busy_violations == 0

        # start_outbound resumes a clean FIFO drain — nothing dropped/reordered.
        await runner.start_outbound("out")
        await _until_stat(store, OutboxStatus.DONE.value, 4)
        assert rec.delivered == [f"p{i}" for i in range(4)]
    finally:
        await runner.stop()
...
def _raw_values_provider(root: Path, environment: str = "dev") -> Callable[[], dict[str, Any]]:
    ...
    file's path), so a provider that already raised ``WiringError`` would exercise the pass-through
    arm instead of the guard, and would pass even with the engine-side guard deleted. This is the
    shape that produced the raw ``TOMLDecodeError``.
    """
...
async def test_reload_wraps_an_unreadable_env_value_file_as_a_wiring_error(tmp_path: Path) -> None:
    """BACKLOG #1652. ``reload`` re-reads the value file through the provider, and that call was
    unguarded: a malformed ``environments/<env>.toml`` raised ``TOMLDecodeError`` -- a ``ValueError``,
    NOT a ``WiringError`` -- straight out of the reload, which the API answers as a 500 with no audit
    row. It must arrive as ``WiringError``, carrying no file CONTENT, with the live graph untouched.

    RED when: the guard in ``reload_detail`` is removed (``pytest.raises(WiringError)`` sees a bare
    ``TOMLDecodeError``). The first reload is the negative control -- a guard that raised
    unconditionally would satisfy the second assertion and fail that one.
    """
    inbox, outdir, live = tmp_path / "in", tmp_path / "out", tmp_path / "live"
    _write_valid_config(live, inbox, outdir)
...
async def test_reload_passes_a_providers_own_wiring_error_through_unwrapped(tmp_path: Path) -> None:
    """The CLI provider wraps its OWN read and names the value file, so ``reload`` must re-raise that
    unchanged. Nesting a second sentence around it would make the operator read the same diagnosis
    twice, and the path is the part they act on."""
    inbox, outdir, live = tmp_path / "in", tmp_path / "out", tmp_path / "live"
    _write_valid_config(live, inbox, outdir)
    message = (
        "could not read environment values from /cfg/environments/dev.toml: TOMLDecodeError: x"
    )
    armed = False

    def provider() -> dict[str, Any]:
        # Engine.create() calls the provider once to seed the initial values, so it must succeed
        # there; only the reload's re-read fails.
        if armed:
