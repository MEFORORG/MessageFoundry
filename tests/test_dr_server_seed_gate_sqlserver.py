...
test_adr0071_dispatch_wiring_sqlserver.py. Requires the `sqlserver` extra + the scripts/dev SS container
(2022/2025). Run pytest FROM the worktree (CWD shadows the editable install)."""
...
# SQLite store with .backend monkeypatched — this drives it against the REAL SQL Server row type
# (dict row.keys()/row["detail"]) that _latest_backup_archive relies on. Active only when
# [dr].restore_token is set.
