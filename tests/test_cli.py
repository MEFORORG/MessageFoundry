...
def test_serve_ui_offloopback_refusal_prescribes_a_config_that_loads(
    ...
    [security] field. Only running the prescribed config finds it, so arm 2 below is a NEGATIVE CONTROL
    that fails on the pre-repair message and passes on this one.
    """
    offbox = (
    ...
    assert _l5b_serve(tmp_path, monkeypatch, offbox) == 2
    refusal = next(
        line for line in capsys.readouterr().err.splitlines() if _UI_OFFLOOPBACK_REFUSAL in line
    )
    ...
    assert "local_access_only" not in refusal, (
        ...
        f"non-loopback listen_address that is the only way to reach this gate: {refusal!r}"
    )

    # 2. NEGATIVE CONTROL. The remediation the message used to give, applied to the config that
    #    tripped it, dies at load -- and never reaches the /ui gate to be cleared.
    dead = offbox.replace("local_access_only = false", "local_access_only = true")
    assert _l5b_serve(tmp_path, monkeypatch, dead) == 2
    ...
    fixed = offbox.replace('listen_address = "0.0.0.0"', 'listen_address = "127.0.0.1"')
    rc = _l5b_serve(tmp_path, monkeypatch, fixed)
