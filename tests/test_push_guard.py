...
requires a PR and 12 status checks, but ``enforce_admins`` is false, so the repository owner bypasses
all of it. The realistic trigger is one click on VS Code's Sync button while the current branch happens
to be ``main``.
