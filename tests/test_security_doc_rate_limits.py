...
def _charges(tree: ast.AST, limiter: str) -> bool:
    ...
    lookup whose result is called, either in place or after binding it to a name.

    The lookup form is how ``_auth.py`` already reaches ``allow_reauth_attempt`` across the
    engine/console wheel skew. A lookup that is never called charges nothing. The bound-name case
    is not scope-aware: a call to that name anywhere in ``tree`` counts.
    """
    ...
    if any(isinstance(node, ast.Call) and id(node.func) in lookups for node in nodes):
        return True
    bound = {
        target.id
        for node in nodes
        ...
        and id(node.value) in lookups
        for target in (node.targets if isinstance(node, ast.Assign) else [node.target])
        if isinstance(target, ast.Name)
    }
    return bool(bound and calls_to(tree, bound, bare_only=True))
...
def test_console_charge_probe_can_fail() -> None:
    ...
    if call not in real[_CONSOLE_AUTH_KEY]:
        if _console_charges_admin_write(real):
            pytest.fail(
                f"the console still charges allow_admin_write, but not as `{call}` in "
                f"{_CONSOLE_AUTH_KEY}; re-aim this mutation rather than letting it skip"
            )
    ...
    survivors = sorted(key for key, src in mutated.items() if "allow_admin_write" in src)
    assert survivors, (
