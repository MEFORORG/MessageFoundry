...
"""Live SQL Server proof of the DR-7 server-DB config-only backup (ADR 0049 AC-7, BACKLOG #52/#60): a
...
SQLSERVER)`` on a SQLite store — its docstring concedes the dr_backup row is asserted on SQLite only),
this exercises the genuine path on a real SqlServerStore: ``_is_server_db()`` is true because the store's
``backend`` really is SQLSERVER, ``snapshot_to`` really raises ``DbaDelegatedError`` (never called here —
the config-only branch skips it), and the audit row is written to the real backend.

Gated: skipped unless MEFOR_TEST_SQLSERVER is set (+ MEFOR_STORE_* connection env), like the seed-gate
quartet (test_dr_server_seed_gate_sqlserver.py). Requires the `sqlserver` extra + the scripts/dev SS
container (2022/2025). Run pytest FROM the worktree (CWD shadows the editable install)."""
