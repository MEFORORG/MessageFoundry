...
async def _aborts(store: MessageStore) -> list[dict[str, str]]:
    """The parsed ``{"kind", "reason"}`` detail of every ``dr_activation_aborted`` row, so a test can
    assert on the COUNT of refusals and on the reason each one names -- not only that something
    raised. A gate that refuses twice, or for the wrong reason, passes an exception-only assertion."""
    return [
        json.loads(r["detail"])
        for r in await store.list_audit(limit=50)
        if r["action"] == "dr_activation_aborted"
    ]
...
async def test_config_only_seed_on_sqlite_aborts(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert not coord.active and not state["active"]
        # Exactly ONE refusal, and it is this gate's: the reason names the missing store counts, not
        # the zero-message wording the count compare would have produced had it been reached.
        aborts = await _aborts(dr_store)
        assert len(aborts) == 1, aborts
        assert aborts[0]["kind"] == "seed" and "NO store row counts" in aborts[0]["reason"]
...
async def test_empty_seed_and_empty_store_names_both_readings(tmp_path: Path) -> None:
    ...
    try:
        ...
        # The control on the fixture: a FULL archive of an empty store still declares row counts
        # (every table in the schema, at zero), so this case is not the config-only one above.
        assert empty.row_counts and not empty.config_only
    finally:
        await primary.close()
    dr_store = await MessageStore.open(tmp_path / "dr.db", cipher=make_cipher(key))
    try:
        ...
        aborts = await _aborts(dr_store)
        assert len(aborts) == 1 and "cannot be told apart" in aborts[0]["reason"], aborts
    finally:
        await dr_store.close()
