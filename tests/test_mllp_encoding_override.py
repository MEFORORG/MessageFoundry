...
async def test_send_without_override_is_byte_identical() -> None:
    # No override -> the receiver sees the exact original bytes (the regression guard for back-compat).
    seen = await _receive_one(ADT_DEFAULT, {})
    assert seen == ADT_DEFAULT.encode("utf-8")


async def test_send_with_override_reencodes_on_the_wire() -> None:
    seen = await _receive_one(ADT_DEFAULT, {"encoding_characters": ALT_OVERRIDE})
