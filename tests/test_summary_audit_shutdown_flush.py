...
async def test_the_open_window_is_flushed_at_shutdown(tmp_path: Path) -> None:
    ...
    async with app.router.lifespan_context(app):
        ...
        live = list(await store.list_audit(action="summary_access"))
        assert live == [], (
...
async def test_a_flush_failure_does_not_abort_the_teardown(
    ...
    Two assertions: the lifespan's context manager EXITS, and ``engine.stop()`` still ran. A
    teardown that raises here skips ``engine.stop()``, and the cost of that is the process, not the
    ...
    try:
        async with app.router.lifespan_context(app):
            ...
            monkeypatch.setattr(engine, "stop", _spy_stop, raising=True)
            monkeypatch.setattr(app.state.summary_auditor, "flush", _boom, raising=True)
            await app.state.summary_auditor.note(engine.store, "alice", None, 3, time.time())
    finally:
        # If the guard regresses, the error leaves the lifespan before engine.stop(). Stop it here,
        # so the store's non-daemon worker cannot hang the test process at exit; the test still fails.
        if real_stop and not stopped:
            await real_stop[0]()
    assert stopped == [True], "the flush failure skipped engine.stop()"


async def test_one_failed_window_does_not_drop_the_others() -> None:
    """``flush`` clears its windows before writing them, so it must attempt EVERY one.

    Stopping at the first failed write would drop each later window with no trace. The failures are
    raised together afterwards, so the lifespan guard still logs that rows were lost.
    """
    ...
    class _FailFirst:
        ...
        async def record_audit(
            ...
            *,
            ...
            if not self._failed:
                self._failed = True
                raise RuntimeError("PROBE: deliberate audit write failure")
            self.written.append(actor)

    fake = _FailFirst()
    store = cast(Store, fake)
    coalescer = _SummaryAuditCoalescer()
    now = 7200.0  # one fixed instant, so neither note rolls the other's window over
    await coalescer.note(store, "alice", None, 1, now)
    await coalescer.note(store, "bob", None, 1, now)
    assert fake.written == [], "a window emitted before the flush, so this test proves nothing"

    with pytest.raises(ExceptionGroup) as caught:
        await coalescer.flush(store)
    assert fake.written == ["bob"], f"a failed window dropped the ones after it: {fake.written}"
