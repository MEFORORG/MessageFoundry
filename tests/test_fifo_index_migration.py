...
async def _downgrade_to_old_indexes(store: Any) -> None:
    ...
    On the server backends the simulation must also DELETE the ``schema_meta`` marker (ADR 0064): a
    real pre-B10 database predates the marker entirely, whereas dropping indexes out-of-band on a
    marker-current DB is exactly the documented drift the fast-path does NOT heal — the reopen would
    (correctly) skip the DDL and this test would be asserting the wrong scenario."""
    backend = store._test_backend
    stmts = [
    ...
    if backend == "sqlite":
        ...
        # (No schema_meta on SQLite — its open-path DDL is unchanged by ADR 0064.)
        for s in stmts:
            await store._db.execute(s)
        await store._db.commit()
    elif backend == "postgres":
        async with store._pool.acquire() as conn:
            for s in [*stmts, "DELETE FROM schema_meta"]:
