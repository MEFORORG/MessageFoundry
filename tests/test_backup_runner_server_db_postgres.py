...
"""DR-1 sub-test B (Postgres) — the server-DB DR backup boundary against a LIVE Postgres store.

The existing ``tests/test_backup_runner.py::test_server_db_is_config_only`` fakes the backend via
``object.__setattr__(store, "backend", StoreBackend.POSTGRES)`` on a SQLite store — it never exercises
the REAL ``PostgresStore.snapshot_to``. This drives the actual backend behavior:

* ``PostgresStore.snapshot_to`` raises :class:`DbaDelegatedError` (DB-tier backup is DBA-delegated —
  ``pg_dump`` / PITR — never reimplemented in the engine, BACKLOG #52), and
...
``tests/test_dr_server_seed_gate_postgres.py``. Requires the `postgres` extra + the scripts/dev PG
container. Run pytest FROM the worktree (CWD shadows the editable install). No failover rig."""
...
async def test_backup_runner_falls_back_to_config_only(store: Any, tmp_path: Path) -> None:
    # End-to-end: the runner classifies the live Postgres store as server-DB and produces a CONFIG-ONLY
    # archive (no DB snapshot) that restore-verifies PASS — the AC-7 fallback, proven against the real
    # backend rather than a monkeypatched SQLite store.
    cfg = tmp_path / "config"
    cfg.mkdir()
