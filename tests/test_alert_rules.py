...
def test_rule_refuses_the_retired_bootstrap_admin_expiring_event() -> None:
    # ADR 0183 Amendment A, Wave 3 (BACKLOG #1136): no default account exists, so nothing emits its
    # expiry reminder. A rule naming it would never match, so it is refused like any unknown event.
    with pytest.raises(ValidationError, match="event_type"):
