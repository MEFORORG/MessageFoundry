...
async def _run_once(engine_index_base: int = 0) -> MultiShardReport:
...
async def test_multishard_engine_index_base_offsets_lane_tags() -> None:
    # The WS-B de-confound instrument: a second orchestrator PROCESS drives DISJOINT lanes on the
    # same store via `engine_index_base` (one process has a measured ~457 msg/s ACK ceiling, so a
    # high-aggregate run must be split). Pin: with base=3, this run's engines tag their lanes E3/E4
    # (name prefix, node id, AND the attribution's own tags) — the exact disjointness a concurrent
    # base=0 peer relies on — and the per-engine isolation proof still holds.
    report = await _run_once(engine_index_base=3)
    rec = report.records[0]
    assert {e.name_tag for e in rec.per_engine} == {"E3", "E4"}
    assert {e.node_id for e in rec.per_engine} == {"ms-e3", "ms-e4"}
    assert not rec.any_cross_engine_steal
    for e in rec.per_engine:
        assert e.inbound_rows == _COUNT_PER_ENGINE, e
        assert e.foreign_rows == 0, e
        assert e.reads > 0, e
