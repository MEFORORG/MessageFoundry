...
def test_the_installed_selfheal_payload_matches_the_committed_source() -> None:
    ...
    assert not drifted, (
        ...
        f"passes, because everything else drives the source.\n"
        f"WORK OUT WHICH COPY IS OLDER FIRST. This is a USER-SCOPE file shared by every session on the "
        f"box, so installing from a checkout older than the installed payload downgrades it everywhere, "
        f"for every account, at once:\n"
        f"    git log --oneline -5 -- {SOURCE.relative_to(ROOT).as_posix()}\n"
        f"Only once THIS checkout is confirmed the newer of the two, from a PLAIN terminal (the "
        f"installer refuses under CLAUDECODE, deliberately):\n"
