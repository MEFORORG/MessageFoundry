...
class _ReleaseOutageStore:
    """``release_claimed`` fails ``fail_times`` times, then succeeds. Claims always fail. The sweep's
    ``list_fifo_lanes`` answers ``start()``'s first sweep, then fails if ``sweep_fails``."""

    def __init__(self, *, fail_times: int, sweep_fails: bool = False) -> None:
        self.calls = 0
        self.fail_times = fail_times
    ...
    async def list_fifo_lanes(self, *a: Any, **k: Any) -> list[tuple[str, float]]:
        self.sweeps += 1
        if self.sweep_fails and self.sweeps > 1:
            raise RuntimeError("simulated store outage: database is locked")
        return []


async def test_a_replacement_claimers_release_retry_backs_off_its_traceback(
    caplog: pytest.LogCaptureFixture, monkeypatch: pytest.MonkeyPatch
) -> None:
    """BACKLOG #1609's replacement claimer retries releasing its predecessor's rows every
    _CLAIM_ERROR_BACKOFF_SECONDS while the store is down. That is the per-pass shape #1844 bounds,
    so it goes on the same backoff. #1609's own line for the successful release is unchanged."""
    caplog.set_level(logging.INFO, logger=_DISPATCHER_LOGGER)
    monkeypatch.setattr(stage_dispatcher, "_CLAIM_ERROR_BACKOFF_SECONDS", 0.0)
    store = _ReleaseOutageStore(fail_times=5)
    d = _dispatcher(store)
    claimer = d._claimers[0]
    claimer.orphan_ids.extend(["r-1", "r-2"])
    await asyncio.wait_for(d._adopt_abandoned("claimer-0", claimer), timeout=10.0)

    assert store.calls == 6 and claimer.orphan_ids == []
    faults = [
        r
        for r in _faults(caplog, _DISPATCHER_LOGGER, logging.WARNING)
        if "could not release" in r.getMessage()
    ]
    assert [_count(f) for f in faults] == [1, 2, 4]
    assert all(_carries_a_traceback(f) for f in faults)
    assert _recoveries(caplog, _DISPATCHER_LOGGER) == [
        "StageDispatcher ingress claimer-0 release recovered after 5 consecutive failures"
    ]
    assert claimer.release_faults == FailureRun()
    assert (
        "StageDispatcher ingress claimer-0 released 2 row(s) its predecessor claimed but never "
        "dispatched"
    ) in caplog.text


async def test_a_real_stop_mid_outage_closes_every_open_run(
    caplog: pytest.LogCaptureFixture, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Through stop() itself, which CANCELS the claimer: a run held in a local of the cancelled
    call would never be closed. The claim, release and sweep runs each get their closing line."""
    caplog.set_level(logging.INFO, logger=_DISPATCHER_LOGGER)
    monkeypatch.setattr(stage_dispatcher, "_CLAIM_ERROR_BACKOFF_SECONDS", 0.01)
    store = _ReleaseOutageStore(fail_times=10**6, sweep_fails=True)
    d = _dispatcher(store)
    claimer = d._claimers[0]
    for _ in range(3):
        await d._claim_and_dispatch(claimer, [])
    claimer.orphan_ids.append("r-1")  # as a dead predecessor leaves it; the claimer adopts at start
    await d.start()
    try:
        await _until(lambda: store.calls >= 3 and d._sweep_faults.count >= 2)
    finally:
        await d.stop()
    ...
    assert not _recoveries(caplog, _DISPATCHER_LOGGER)
    assert claimer.faults == claimer.release_faults == d._sweep_faults == FailureRun()


async def test_a_respawned_claimer_continues_its_predecessors_fault_run(
    caplog: pytest.LogCaptureFixture, monkeypatch: pytest.MonkeyPatch
) -> None:
    """A supervisor respawn (#1609) replaces the TASK; the partition, and its run, outlive it. A
    reset there would re-open the schedule at 1 and write a fresh traceback on every death."""
    caplog.set_level(logging.INFO, logger=_DISPATCHER_LOGGER)
    monkeypatch.setattr(stage_dispatcher, "_CLAIM_ERROR_BACKOFF_SECONDS", 0.0)

    class _Store(_ClaimOutageStore):
        async def list_fifo_lanes(self, *a: Any, **k: Any) -> list[tuple[str, float]]:
            return []  # start()'s first sweep finds no lane, so the claimer idles

    d = _dispatcher(_Store(fail_times=99))
    claimer = d._claimers[0]
    for _ in range(3):
        await d._claim_and_dispatch(claimer, [])
    await d.start()
    try:
        before = d.respawns
        assert claimer.task is not None
        claimer.task.cancel()  # cancellation is not a death; kill it with an exception instead

        async def die() -> None:
            raise RuntimeError("injected claimer death")

        dead = asyncio.ensure_future(die())
        claimer.task = dead
        dead.add_done_callback(lambda t: d._on_task_done(0, t))
        await _until(lambda: d.respawns == before + 1)
        assert claimer.faults.count == 3  # the run carried across the respawn
    finally:
        await d.stop()
