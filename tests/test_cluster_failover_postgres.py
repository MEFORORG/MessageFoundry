...
async def test_non_promotable_warm_dr_stays_passive_across_handover(coords) -> None:
    ...
    # row on the real Postgres clock. The DR node must NEVER take the lease — not while a promotable node
    # holds it, and CRUCIALLY not during the open handover window after the leader crashes and its lease
    # ages out (the exact moment the fake-pool unit test asserts but never against a real DB clock). The
    # promotable standby completes the real handover instead. This is the warm-DR guarantee end to end.
    make, _ = coords
    ...
    # (3) ha1 "crashes": stop renewing and let its lease age out against the DB clock.
...
async def test_preferred_delay0_wins_expired_lease_race_over_delayed_node(coords) -> None:
    # HA-8: the acquire_delay handicap ($4: `lease_expires_at + $4 < clock_timestamp()`) resolved against
    # the REAL Postgres clock — the fake-pool unit tests (tests/test_cluster_lease.py) re-implement this
    # arithmetic in Python and never run the claim SQL. A preferred node (delay=0) and a warm-DR node
    # (delay=0.5) both race a lease that has been expired for only ~0.15s: the DR node is handicapped OUT
    # (0.15s < 0.5s → its stricter predicate `expires_at + 0.5 < now` is false), while the preferred node
    # takes over at once — and then KEEPS it (the DR node still can't take a live, renewed lease).
    make, _ = coords
    ll = make("L")
    ...
    # (3) The DR node races the ~0.15s-expired lease FIRST and is handicapped out: `expires_at + 0.5 < now`
    # is false, so its take-over branch never fires. The preferred node then claims the same expired lease.
    await dr._maintain_leadership()
    assert (
        dr.is_leader() is False
    )  # 0.15s expired < 0.5s handicap → the real $4 predicate rejects DR
