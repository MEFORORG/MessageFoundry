...
async def test_ingress_stop_policy_halts_and_alerts(store: MessageStore, tmp_path: Path) -> None:
    ...
    sink = _RecordingAlertSink()
    # per_lane: asserts the per-inbound router worker halts (pooled handles STOP via the dispatcher).
    runner = RegistryRunner(
        reg,
        store,
        poll_interval=0.02,
...
async def test_ingress_stop_then_restart_rearms_worker(store: MessageStore, tmp_path: Path) -> None:
    ...
    reg = _raising_router_registry(inbox, outdir, route)
    # per_lane: asserts the per-inbound router worker halts + re-arms on restart (pooled path differs).
    runner = RegistryRunner(
        reg,
        store,
        poll_interval=0.02,
...
#
# start/stop/restart_outbound + the tri-state outbound_status (running/stopping/stopped) + the
# quiescence-gated outbound_quiesced, exercised in BOTH claim modes (the DEFAULT pooled StageDispatcher
# AND the per_lane delivery worker). Pause is COOPERATIVE: it retains queued rows PENDING (no drop /
# reorder), never task.cancels, and 'stopped' means ZERO in-flight (the require-stopped-before-purge
# precondition).


class _Recorder:
    """Destination connector that records each delivered payload (in FIFO order) and returns fast."""
...
async def _outbound_status_counts(store: MessageStore, dest: str) -> dict[str, int]:
    """OUTBOUND-stage queue rows for ``dest``, counted by status (a lane-scoped INFLIGHT/PENDING probe)."""
    cur = await store._db.execute(
        "SELECT status, COUNT(*) AS n FROM queue WHERE stage=? AND destination_name=? GROUP BY status",
        (Stage.OUTBOUND.value, dest),
    )
    return {r["status"]: r["n"] for r in await cur.fetchall()}


def _outbound_only_registry(outdir: Path) -> Registry:
    reg = Registry()
    reg.add_outbound(
        OutboundConnection("out", ConnectionSpec(ConnectorType.FILE, {"directory": str(outdir)}))
    )
    return reg


@pytest.mark.parametrize("claim_mode", ["per_lane", "pooled"])
async def test_outbound_stop_retains_rows_then_resume_drains_fifo(
    store: MessageStore, tmp_path: Path, claim_mode: str
) -> None:
    """stop_outbound halts DELIVERY while RETAINING queued rows PENDING (never dropped/reordered) and
    ...
    try:
        assert runner.outbound_running("out") is True
        assert runner.outbound_status("out") == "running"

        # Deliver 3 rows to prove the lane is live.
        for i in range(3):
        ...
        # Stop: returns fast; outbound_running flips False immediately (operator intent).
        await runner.stop_outbound("out")
        assert runner.outbound_running("out") is False
        # It reaches 'stopped' (quiesced — zero in-flight) since the Recorder held nothing mid-send.
        assert await _poll(lambda: runner.outbound_status("out") == "stopped")
        assert runner.outbound_quiesced("out") is True

        # Enqueue 4 more while stopped: RETAINED PENDING (never delivered/dropped); a nudge must NOT
        ...
        counts = await _outbound_status_counts(store, "out")
        assert counts.get(OutboxStatus.INFLIGHT.value, 0) == 0  # ZERO INFLIGHT while stopped
        assert counts.get(OutboxStatus.PENDING.value, 0) == 4  # the 4 new rows retained PENDING
        assert len(rec.delivered) == 3  # none delivered while paused
        assert runner.outbound_running("out") is False  # notify_work did NOT resume it

        # Resume: connector kept WARM (same object), retained rows drain in FIFO order.
        assert runner._destinations["out"] is rec
        await runner.start_outbound("out")
        assert runner.outbound_running("out") is True
        assert runner.outbound_status("out") == "running"
        await _until_stat(store, OutboxStatus.DONE.value, 7)
        assert rec.delivered == [f"p{i}" for i in range(7)]  # strict FIFO across the pause
    finally:
        await runner.stop()


async def test_per_lane_stop_outbound_quiesces_after_content_stop_exited_worker(
    store: MessageStore, tmp_path: Path
) -> None:
    """per_lane REGRESSION: an outbound whose delivery worker already EXITED via an
    InternalErrorPolicy.STOP content-stop (a clean ``return`` — no exception, so ``_on_worker_done``
    does NOT respawn) has NO live worker to hit the loop-top pause gate. stop_outbound must still detect
    the absent/done worker and signal quiescence DIRECTLY (already zero in-flight), so the lane reaches
    'stopped' (purge permitted) instead of hanging in 'stopping' forever."""
    inbox, outdir = tmp_path / "in", tmp_path / "out"
    inbox.mkdir()
    reg = _stop_registry(inbox, outdir, InternalErrorPolicy.STOP)
    sink = _RecordingAlertSink()
    ...
    (inbox / "a.hl7").write_bytes(ADT.encode("utf-8"))
    try:
        # Drive the content-STOP: the delivery worker calls send() → ValueError → STOP policy → the
        # worker returns. It ends done, with NO exception, so _on_worker_done leaves it un-respawned.
        await _until(lambda: bool(sink.stopped))
        await _until(lambda: runner._workers["file_out"].done())
        assert runner._workers["file_out"].exception() is None  # clean return → not respawned

        # THE FIX: stop_outbound on a lane whose worker already exited signals quiescence directly (no
        # loop-top gate will ever fire), so status is 'stopped' — never wedged 'stopping'.
        await runner.stop_outbound("file_out")
        assert runner.outbound_quiesced("file_out") is True
        assert runner.outbound_status("file_out") == "stopped"
    finally:
        await runner.stop()


async def test_pooled_stop_outbound_quiesces_after_content_stop(
    store: MessageStore, tmp_path: Path
) -> None:
    """pooled parity for the per_lane fix above (confirming no pooled change was needed): once an
    InternalErrorPolicy.STOP content-stop drives the OUTBOUND dispatcher lane to STOPPED, stop_outbound
    (→ ``pause_lane`` on a STOPPED lane) reaches PAUSED and fires ``on_lane_paused`` → quiescence, so the
    lane reports 'stopped'. The dispatcher already handled this."""
    inbox, outdir = tmp_path / "in", tmp_path / "out"
    inbox.mkdir()
    reg = _stop_registry(inbox, outdir, InternalErrorPolicy.STOP)
    sink = _RecordingAlertSink()
    ...
    (inbox / "a.hl7").write_bytes(ADT.encode("utf-8"))
    try:
        await _until(lambda: bool(sink.stopped))  # dispatcher OUTBOUND lane reached STOPPED
        await runner.stop_outbound("file_out")
        assert await _poll(lambda: runner.outbound_quiesced("file_out") is True)
        assert runner.outbound_status("file_out") == "stopped"
    finally:
        await runner.stop()
...
@pytest.mark.parametrize("claim_mode", ["per_lane", "pooled"])
async def test_outbound_stop_zero_inflight_with_hung_destination(
    store: MessageStore, tmp_path: Path, claim_mode: str
) -> None:
    """stop_outbound RETURNS FAST even against a hung destination (never hangs the caller): while the one
    in-flight head is still in send(), status is 'stopping' (not yet quiesced) — so a purge would stay
    ...
    class _Gate:
        ...
        async def send(self, payload: str) -> None:
            await self.event.wait()  # hang until released
            self.delivered.append(payload)
    ...
    gate = _Gate()
    runner._destinations["out"] = gate
    try:
        await store.enqueue_message(
            channel_id="c", raw=ADT, deliveries=[("out", "p0")], source_type="file"
        )
        runner.notify_work()

        # Wait until the head is claimed + in flight (INFLIGHT), blocked in the hung send().
        async def _one_inflight() -> bool:
            counts = await _outbound_status_counts(store, "out")
            return counts.get(OutboxStatus.INFLIGHT.value, 0) == 1

        deadline = asyncio.get_running_loop().time() + 3.0
        while asyncio.get_running_loop().time() < deadline:
            if await _one_inflight():
                break
            await asyncio.sleep(0.01)
        assert await _one_inflight()  # the head is mid-delivery (INFLIGHT)

        # Stop RETURNS FAST despite the hung send; the in-flight head keeps the lane in 'stopping'.
        await asyncio.wait_for(runner.stop_outbound("out"), timeout=1.0)
        assert runner.outbound_running("out") is False
        assert runner.outbound_status("out") == "stopping"  # in-flight head not yet resolved
        assert runner.outbound_quiesced("out") is False  # purge would stay 409-blocked here

        gate.event.set()  # the head finishes -> the lane quiesces to zero in-flight
        assert await _poll(lambda: runner.outbound_status("out") == "stopped")
        assert runner.outbound_quiesced("out") is True
        counts = await _outbound_status_counts(store, "out")
        assert counts.get(OutboxStatus.INFLIGHT.value, 0) == 0  # ZERO INFLIGHT once 'stopped'
    finally:
        gate.event.set()
        await runner.stop()
...
@pytest.mark.parametrize("claim_mode", ["per_lane", "pooled"])
async def test_outbound_resume_drains_backed_off_head(
    store: MessageStore, tmp_path: Path, claim_mode: str
) -> None:
    """A paused-then-resumed outbound whose in-flight head had backed off (RETRY -> re-pended PENDING) is
    eventually DRAINED on resume, never stranded. Cooperative pause never cancels, so nothing is left
    INFLIGHT while stopped."""
    reg = Registry()
    reg.add_outbound(
        OutboundConnection(
            "out",
            ConnectionSpec(ConnectorType.FILE, {"directory": str(tmp_path)}),
            retry=RetryPolicy(
    ...
    flaky = _FlakyDestination(
        fail_times=1
    )  # first send fails -> the head backs off (re-pended PENDING)
    runner._destinations["out"] = flaky
    try:
        await store.enqueue_message(
            channel_id="c", raw=ADT, deliveries=[("out", "p0")], source_type="file"
        )
        runner.notify_work()
        # Wait until the head has failed once (remaining consumed) — now backing off PENDING.
        assert await _poll(lambda: flaky.remaining == 0)

        await runner.stop_outbound("out")
        assert await _poll(lambda: runner.outbound_status("out") == "stopped")
        counts = await _outbound_status_counts(store, "out")
        assert counts.get(OutboxStatus.INFLIGHT.value, 0) == 0  # never stranded INFLIGHT
        assert len(flaky.deliveries) == 0  # not delivered yet

        await runner.start_outbound("out")  # resume -> the backed-off head is re-swept + drained
        await _until_stat(store, OutboxStatus.DONE.value, 1)
        assert flaky.deliveries == ["p0"]
    finally:
        await runner.stop()


@pytest.mark.parametrize("claim_mode", ["per_lane", "pooled"])
async def test_restart_outbound_keeps_connector_warm(
    store: MessageStore, tmp_path: Path, claim_mode: str
) -> None:
    """restart_outbound = stop+start in one lock span, keeping the connector WARM (``_destinations``
    ...
    runner._destinations["out"] = rec
    try:
        await runner.restart_outbound("out")
        assert runner._destinations["out"] is rec  # connector NEVER torn down (kept warm)
        assert runner.outbound_running("out") is True  # ends running
        await store.enqueue_message(
            channel_id="c", raw=ADT, deliveries=[("out", "p0")], source_type="file"
        )
        runner.notify_work()
        await _until_stat(store, OutboxStatus.DONE.value, 1)
        assert rec.delivered == ["p0"]
    finally:
        await runner.stop()


async def test_paused_outbound_suppresses_buildup_and_stall_then_lifts(
    store: MessageStore, monkeypatch: pytest.MonkeyPatch
) -> None:
    """A deliberately-paused outbound's growing queue must NOT false-page: the queue_buildup + message_stall
    alerts are suppressed while it is in _outbound_paused, and the suppression LIFTS immediately once it is
    removed (start_outbound) so a genuinely backed-up RESUMED lane still pages."""
    import time as _time
    ...
    # Paused: both alerts are suppressed even though the lane is well over threshold.
    runner._outbound_paused.add("OB")
    await runner._maybe_alert_buildup("OB")
    await runner._maybe_alert_stall("OB")
    assert sink.buildups == []
    assert sink.stalls == []

    # Resumed (removed from _outbound_paused): the suppression lifts immediately -> the still-backed-up
    # lane pages again.
    runner._outbound_paused.discard("OB")
    await runner._maybe_alert_buildup("OB")
    await runner._maybe_alert_stall("OB")
    assert sink.buildups and sink.buildups[0][0] == "OB"
    assert sink.stalls and sink.stalls[0][0] == "OB"
...
async def test_outbound_control_unknown_name_raises_and_reload_lock(store: MessageStore) -> None:
    ...
    try:
        with pytest.raises(KeyError):
            await runner.stop_outbound("nope")  # unknown outbound
        # M-10: stop_outbound blocks on a held reload lock.
        await runner._reload_lock.acquire()
        task = asyncio.ensure_future(runner.stop_outbound("out"))
        await asyncio.sleep(0.05)
