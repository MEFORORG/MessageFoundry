...
class Engine:
    def __init__(
        ...
        *,
        ...
        # SandboxSettings defaults (mode=off, byte-identical). Rendered into a SandboxPolicy passed to
        # every runner this engine builds; read ONCE here (a /config/reload does NOT re-read it —
        # restart to change).
        ...
        # Pooled per-stage claimers (ADR 0066). Reliability-core + read ONCE here; every runner this
        # engine builds inherits them (a /config/reload never re-reads claim_mode — restart to change,
        # exactly like per_lane_wake). claim_mode="pooled" (the default since #744) builds the per-stage
        # StageDispatchers; the "per_lane" opt-out is byte-identical to the pre-ADR-0066 topology and
        # constructs zero pooled objects in the runner.
    ...
    def add_registry(self, registry: Registry) -> RegistryRunner:
        ...
        # no-op — byte-identical, no child ever spawned; mode=subprocess spawns one persistent worker
        # child per inbound that dispatches. ONE policy for the WHOLE graph: [sandbox] is engine-wide
        # and no connection carries a per-connection sandbox field, so the mode chosen here governs
        # every Router and Handler in this process.
        ...
        runner = RegistryRunner(
            ...
            claim_mode=self._claim_mode,
            pooled_claimers_per_stage=self._pooled_claimers_per_stage,
    ...
    async def start(self) -> None:
        ...
        if self._coordinator.reclaims_inflight() and hasattr(self.store, "reclaim_expired_leases"):
            ...
            self._leader_maintenance = LeaderMaintenanceRunner(
                ...
                # WS-C: reclaimed rows re-pend while the workers sleep on the long per-lane-wake
                # backstop — nudge every stage so they re-claim promptly. Lazy read (the runner is
                # (re)built by _start_graph after this); recovery never depends on the nudge (the
                # backstop bounds a missed one to added latency).
                on_reclaimed=lambda: (
