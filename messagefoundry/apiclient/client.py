...
#: How much of a failed revoke's error text reaches the log (see EngineClient._end_replaced_session).
_REVOKE_LOG_CHARS = 300
    ...
    **The default ``base_url`` is https, because the engine serves TLS by default** (ADR 0172). A
    ...
    def __init__(
        ...
        *,
        ...
        #: The action a per-action step-up 403 named in ``X-Step-Up-Action`` (ADR 0077), stashed just
        #: before the handler runs so :meth:`reauth` can bind the fresh proof to it. None for a plain
        #: session-window step-up (no header) — then :meth:`reauth` posts no ``purpose``, unchanged.
        self._pending_step_up_action: str | None = None
    ...
    def _request(
        ...
        **kw: object,
        ...
        # rather than a side request so it keeps every bound below and the cleartext refusal. It
        # disarms both retries: their handlers elevate the HELD session, not the one sent here.
        if _bearer is not None:
            _allow_step_up = _allow_mfa = False
        bearer = self._token if _bearer is None else _bearer
        ...
        if (
            ...
            # ADR 0077: a per-action step-up names the action in X-Step-Up-Action; stash it BEFORE the
            # handler runs so the reauth() it calls binds the fresh proof to that action. A plain
            # session-window step-up omits the header → None → reauth() posts no purpose (unchanged).
    ...
    def config_provenance(self) -> ConfigProvenance:
        """Provenance of the loaded config graph — running commit + on-disk DRIFT flag (ADR 0041 D1).
        Non-secret (a content hash + a commit sha); gated by ``monitoring:read`` like :meth:`status`."""
        return _decode(self._get("/config/provenance"), ConfigProvenance)
    ...
    def _end_replaced_session(self, prior: str) -> None:
        ...
        ``_REVOKE_LOG_CHARS``, because the detail falls back to a whole response body. And it is
        logged with ``%r``, so a CR or LF in it cannot forge a log line.
