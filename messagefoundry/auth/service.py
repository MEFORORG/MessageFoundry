...
class AuthService:
    ...
    async def confirm_mfa_enrollment(
        ...
        (or a time-step already consumed -- single-use, BACKLOG #1021) is ``ok=False`` with none.
    ...
    async def finish_webauthn_registration(
        ...
        *,
        ...
        code); raises :class:`ValueError` for flow errors with safe, renderable messages (AD account,
        bad label, expired ceremony, duplicate label/credential). On success the enrolling session is
        marked MFA-verified and re-keyed (exact :meth:`confirm_mfa_enrollment` parity, ASVS 7.2.4) —
        **no recovery codes are minted** (ADR 0068 decision 5).
