...
def _provenance_text(prov: ConfigProvenance | None) -> str:
    """One-line config provenance for the Engine box: 'abc1234 — clean|DRIFTED', or '—' when the engine
    hasn't loaded a graph / the endpoint isn't available (older engine / not permitted)."""
    if prov is None or not prov.loaded:
        return "—"
    if prov.git_head:
        ident = prov.git_head[:7]
    elif prov.fingerprint:
        ident = prov.fingerprint[:12]
    else:
        ident = "loaded"
    return f"{ident} — {'DRIFTED' if prov.drift else 'clean'}"
...
class _Snapshot:
    ...
    ``provenance`` None ⇒ the config-provenance endpoint isn't available / not permitted."""

    service_state: str
    status: SystemStatus | None
    cluster: tuple[ClusterStatus, ClusterNodeList] | None
    provenance: ConfigProvenance | None
    error: str | None
...
class EngineStatusPage(QWidget):
    ...
    def _fetch(self) -> _Snapshot:
        ...
        try:
            ...
            return _Snapshot(svc, None, None, None, str(exc))
        cluster: tuple[ClusterStatus, ClusterNodeList] | None
        try:
        ...
        prov: ConfigProvenance | None
        try:
            prov = self._client.config_provenance()
        except ApiError:
            prov = None  # older engine / not permitted — leave the Config row blank
        return _Snapshot(svc, status, cluster, prov, None)
