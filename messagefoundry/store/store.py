...
class ClaimedHeads:
    ...
    ``contended`` names lanes this call yielded on because something ELSE held the row it wanted --
    not because the lane was empty (BACKLOG #1270). **An absent lane and a contended lane are both
    "nothing claimed", and until this field existed nothing downstream could tell them apart**: the
    dispatcher counted the empty claim, dropped the lane to IDLE with no timer armed, and every
    observable said "there is no work". A lane that has silently stopped claiming reads exactly like
    an idle system.
    ...
    **A backend reports here ONLY what it knows for free.** Empty from a backend that does not
    ...
    reading the occupancy fence carries. Do not write a consumer that treats an empty ``contended``
    as proof the lanes were genuinely idle."""
