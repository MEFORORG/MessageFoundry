...
def _encode_search_params(params: FhirSearchParams) -> str:
    """Encode a structured search into a URL query string, at **both** layers.

    **The URL layer: percent-encoding.** A value can never inject an extra FHIR search *parameter*
    (CWE-88 argument injection, ASVS 1.2.2). ``urlencode(quote_via=quote, safe="")`` encodes **every**
    reserved char in each key/value, so an ``&``/``=``/``#`` in a value becomes ``%26``/``%3D``/``%23``
    and the server parses back exactly the parameters this function was handed — one value stays one
    value. A ``list`` value becomes repeated params (``identifier=a&identifier=b``), which FHIR reads as
    an AND. Returns ``""`` for empty ``params`` (a search of the whole resource type).

    **The FHIR value layer: percent-encoding does not neutralise ``,`` ``|`` ``$``, so the value's
    KIND decides what happens to them** (BACKLOG #1243). A server percent-decodes first and *then*
    reads FHIR's syntax inside the decoded value, so a decode re-forms those separators with their
    meaning live. :func:`~messagefoundry.fhirsearch.resolve_search_pairs` therefore resolves each value
    by kind before it is encoded here, and **refuses** rather than escapes: a plain ``str`` carrying
    one of the three raises, a :class:`~messagefoundry.fhirsearch.FhirToken` splits the system and code
    halves, and a :class:`~messagefoundry.fhirsearch.FhirRaw` is author syntax and rides through. The
    vocabulary, the specification citations and why it refuses live once, on
    :mod:`messagefoundry.fhirsearch` — read that module before passing message data into ``params``."""
    return urllib.parse.urlencode(
        resolve_search_pairs(params), quote_via=urllib.parse.quote, safe=""
    )
...
def _resolve_read_url(
    ...
    **There is exactly one search form, and it is encoded by construction** (ASVS 1.2.2, BACKLOG #1243).
    Every value goes through :func:`_encode_search_params`, which holds the URL layer by percent-encoding
    and the FHIR value layer by resolving each value's kind — see that function's docstring before
...
class FhirLookupExecutor:
    ...
    **PHI/secret-safe:** an error names only routing-safe identifiers (connection, HTTP status, an
    ...
    async def read(
        ...
        before passing message-derived data:** percent-encoding is a URL-layer control only, so a plain
        ``str`` value carrying a FHIR value-layer separator is **refused** here rather than sent.
        ...
        try:
            ...
            # PHI-safe: the raised ValueError names only the offending shape/segment, or a refused
            # parameter key and separator character -- never a query value (BACKLOG #1243).
