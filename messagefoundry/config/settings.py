...
class AuthSettings(_Section):
    ...
    # re-verified its credential -- at login or via POST /me/reauth -- within this many seconds. A
    # LOCAL login that owes no second factor counts as the first verification (sudo-timestamp
    # model); a directory login (Kerberos, OIDC) does not (BACKLOG #1144). Default 5 minutes.
