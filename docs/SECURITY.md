...
at login. A local sign-in that still owes a factor is minted with no window. Browser Windows
SSO (`GET /ui/sso`) and OIDC mint with **no** window (see the Authentication ambience row), so their first
sensitive action forces a step-up. On any pathway, an MFA-pending session gets its window when it proves
a TOTP or recovery code, because `verify_mfa` stamps `reauth_at` too. A passkey assertion
...
needs to re-verify once its window lapses. `POST /me/reauth` re-checks the **local** password (argon2) or performs a **live
...
and OIDC appear as links when enabled. A POST that still carries `provider=ad` is charged to the
...
- **Out-of-band email to the affected user** (gated by `[auth].notify_security_events`, default on; it
  ...
  **old** address so the legitimate owner is alerted even if the change was hostile. **Only an
  administrator's explicit `notify_email` moves the notification address** (`PATCH /users/{id}`, or
  the console user page's Notification address field; BACKLOG #1139, ADR 0182 Amendment A). Saving
  the profile `email` never moves it, so an unrelated save cannot copy the directory's `mail` into
  it or fill a missing one from it. A move writes `user.notify_email_changed` and notifies the old
  address, or the new one when there was none. With no `[alerts]`
