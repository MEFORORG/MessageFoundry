...
at login. A local sign-in that still owes a factor is minted with no window, and so is **every
...
- **Windows SSO (Kerberos)** — optional, experimental. `POST /auth/negotiate` completes a SPNEGO
  ...
  **Browser SSO (L5c, ADR 0068 §9):** `GET /ui/sso` adds the RFC 4559 browser flow over the same
  ...
  since BACKLOG #1144 step 5, which made the approved flip and closed the recorded asymmetry). A **boot-once acceptor preflight** (app lifespan)
