...
- **Out-of-band email to the affected user** (gated by `[auth].notify_security_events`, default on; it
  ...
  alert distribution list). Fired on: account **lockout** and the **first successful login or step-up
  re-auth after ≥3 failed attempts** (suspicious-login signals, 6.3.5); and **password change**, **email change**, **role
...
the threshold per lock cycle, so a domain lockout threshold at or below the engine's can still lock the
domain account. A directory it cannot reach, or one with no such principal, is a refusal but not a
counted failure. The per-actor
...
| 3 | **Per-actor credential-ceremony budget** (`allow_reauth_attempt`) | a session holder guessing a password at the re-proof surface, **before** control 1 locks the account | > 10 ceremonies per acting **user**, per 60 s. **No global dimension** (`glob=0`) | *the same* `[auth].login_rate_limit_enabled` | control 1 only — `POST /me/reauth` and `POST /me/password` still feed and honour the lockout and run one at a time per account, so they check 5 wrong guesses before each 15-minute lock. The sign-in leg does not serialize, so a parallel burst there can have more checked before the lock lands |
...
per-actor ceremony budget, which has no global dimension. The re-proofs run one at a time per account,
so they check 5 wrong guesses before each 15-minute lock. The default
