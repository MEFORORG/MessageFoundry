...
| **HTTP intake** | `api_key` / `bearer`: none, the shared secret is phishable like any password; `mtls_subject`: the client certificate's | `api_key` / `bearer`: none beyond TLS, and a listener without `tls` sends the secret in cleartext; `mtls_subject`: the TLS handshake | **none** in the store — `intake_api_key` / `intake_api_key_next` must be `env()` references, never inline; for `mtls_subject` the engine holds only `tls_ca_file` and the subject list | none, structurally | change the `env()` secret (the `intake_api_key_next` slot avoids an outage) or drop the subject from `intake_client_subjects`; under `[security].enforcement = enforce` an `mtls_subject` listener must also set `tls_crl_file`, or declare `tls_revocation_attested = true`, or it is refused at start |
