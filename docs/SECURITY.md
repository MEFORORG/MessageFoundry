...
- **Time since the IdP sign-in is bounded too** (BACKLOG #1150). Every authorization request sends
  `max_age` = `[auth].oidc_max_age_seconds` (default **43200 s**, 12 h, **no off switch**; the range
  is in [CONFIGURATION.md](CONFIGURATION.md)). The IdP re-authenticates the user only if its own
  sign-in is older than that, so single sign-on survives inside the window. The `id_token` must
  then carry `auth_time`: a token without it, or one leaving under a minute of the bound, is
  refused (`auth_time_missing` / `auth_time_stale`). The session also ends at
  `auth_time + oidc_max_age_seconds` when that is sooner than the other caps. That cap matters
  ...
  `auth_time` only when configured to, so check a captured token with
  `messagefoundry verify --section federation` before enabling federation.
