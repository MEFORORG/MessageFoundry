...
on the engine's own row, and a locked row is refused without binding. The engine **never locks the
directory account**; a directory it cannot reach is a refusal but not a counted failure. The per-actor
...
| 3 | **Per-actor credential-ceremony budget** (`allow_reauth_attempt`) | a session holder guessing a password at the re-proof surface, **before** control 1 locks the account | > 10 ceremonies per acting **user**, per 60 s. **No global dimension** (`glob=0`) | *the same* `[auth].login_rate_limit_enabled` | control 1 only — `POST /me/reauth` and `POST /me/password` still feed and honour the lockout, so a session holder gets 5 wrong guesses before each 15-minute lock |
...
A directory account's engine row is fed only by the step-up re-bind, which needs a live session (the
lockout-asymmetry note above says why). Control 6 refuses an off-network client before any
...
per-actor ceremony budget, which has no global dimension. For a session holder the bound is 5 wrong
guesses before each 15-minute lock. The default
