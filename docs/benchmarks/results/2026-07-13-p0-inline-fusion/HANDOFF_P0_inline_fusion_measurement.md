...
**Date:** 2026-07-12 · **Continues C7** · **Gates ADR 0099 / ADR 0057** · **Discipline: ADR 0101 (pre-registered
falsifier).** **Zero production code** — harness + config only. This run **authorizes or KILLS** the entire Phase-4
build. Read-only rig work + one ~20-line harness patch. Public DMV/catalog names only; no secrets, IPs, hostnames, PHI.

> **In-repo twin:** this file is mirrored at `docs/benchmarks/HANDOFF_P0_inline_fusion_measurement.md` (PR #985). The
> C4–C7 record it builds on is archived at `docs/benchmarks/results/2026-07-12-throughput-c4-c7/`.

---

## 0. What this settles, and the trap that would fake the answer

The store-side search is closed (ADR 0098): N-sizing insufficient (C5), no convoy (C6), parallelism load-bearing (C7).
The **one** surviving lever is reducing **committed transactions per event** — and its shipped instance, **inline
stage-fusion (ADR 0057), is already in the tree and has never been enabled in a rig run.** P0 measures whether turning
it on moves sustained throughput.

**And the Phase-4 premise itself — "per-message transaction count is the constraint" — has never been tested. P0 tests
...
- The fusion gate is **`if inline and len(names) == 1:`** — verified at `wiring_runner.py:3712`.
- The shardcert default shape is **`H = D = dests = 8`** — verified at `harness/config/shardcert/_shape.py` (`H` and
  `D` both default to `dests`).
...
1. **Engine box back at m7i.4xlarge (16 vCPU).** P0 is A/B'd for comparability against the C5/C6/C7 baseline, which is a
   **16-vCPU** baseline — a same-session contrast across a rig change is not valid. Store box i4i.2xlarge (unchanged,
   `n_sched=8`). *(The box was downsized to 2x while idle — upsize before the first arm.)*
...
`MEFOR_SHARDCERT_DESTS=1`. **`inline`** is a per-inbound bench knob; **`batch_handoff_statements`** is the shipped store
setting (default **True**, `settings.py:950`).
...
`SqlServerStore.committed_txns` (`sqlserver.py:1046`, incremented in `_commit`) is on `/stats` and is **already polled
and summed across shards** by `harness/load/enginepoll.py` — shardcert just never records it (it reports the *modelled*
`3 + 2H + 2D` instead). **The patch (~20 lines): record `Δcommitted_txns` per arm and divide by delivered messages.**
...
- **`inline_fallbacks` must be exactly 0 in arms B and D.** If not, some messages took the split path (FIFO hazard live)
  — **void the arm.** *(If the `inline_fallbacks` counter does not exist yet, add it alongside the txn recorder — it is
  the arming proof for the fused path.)*
...
- **Attribution** (bench-attribution-policy): client isolation, `max_core%` on **both** boxes, verified-nonzero
...
## 9. TEARDOWN
**Do NOT tear down the instances.** Instance lifecycle (stop / terminate / deallocate) is the **owner's** call — an
unsanctioned STOP wipes the instance-store `mfbench` volume. When P0 is banked: **report it and HOLD THE RIG.** The
tempdb-metadata feature stays **ON** unless the owner says otherwise (Phase 4 follow-ups reuse this store config).

## 10. What to send back (`HANDBACK_P0_<date>.md`)
...
## 11. Sources / notes
- The full design + why the other three Phase-4 angles were killed: `docs/benchmarks/PLAN-PHASE4-GROUP-COMMIT.md`.
- The lever's status + the ledger reconciliation: `docs/adr/0099-*.md`, `docs/adr/0057-*.md`.
- The measurement discipline: `docs/adr/0101-*.md`.
- The baseline this A/B's against: the sibling `c5-n8-headroom-2026-07-11/` folder (and its archived copy under
  `docs/benchmarks/results/2026-07-12-throughput-c4-c7/`).
- **Line numbers are current-worktree** (the public mirror lags); the facts, not the lines, are load-bearing and were
  re-verified 2026-07-12. Read-only DMV / public catalog names only; no secrets, IPs, hostnames, ports, or PHI.
