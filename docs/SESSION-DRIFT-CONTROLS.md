...
| Announce-on-join (`announce-session.ps1`) | user | **LIVE** (re-verified 2026-08-03 by receipt *and* by `-Status`). The only **push** control. It **asks** for the peers the model's tools can reach and **sends** for the ones they cannot: from 2026-09-14 a cross-login or unenumerable-surface peer is mailed by the hook itself through `mail.ps1`, because no instruction can conjure a session id outside the caller's namespace. Every decision leaves a receipt, with `msg=` for what the model was asked to send and `mail=` for what the hook sent. Was inert-by-accident for ~13h after merge — see below, the lesson outlived the defect |
...
| B6 | **Mostly done** | `.worktreeinclude` added, so the leak gate's gitignored token list reaches every worktree Claude Code creates itself (`--worktree`, desktop sessions, `isolation: worktree` subagents) — previously only `new.ps1`'s own worktrees got it, and a fresh first-party worktree could not commit at all. **Worktree-first is now the documented default entry point** ([WORKTREES.md](WORKTREES.md) §"Start the session in the worktree", CLAUDE.md §5): create the worktree, start the session **in** it, and never brief a worker to relocate. **The blocker under it is reachable now, not closed.** `isolation: worktree` was unusable for build work because the harness creates a checkout and installs nothing — measured 2026-09-16, **47 of 86** nested worktrees had a usable `.venv` against **21 of 22** siblings — so a session wanting an environment took a sibling and relocated into it, which is the owner prompt this was costing. `scripts/worktree/ensure-venv.ps1` now owns the bootstrap (idempotent, runnable in any worktree) and `new.ps1` calls it; the extras-parity and `constraints.lock` gates moved with the declaration. **Stated exactly: the harness still installs nothing.** Any session can now fix that in one command, but nothing makes it fire, so it still depends on the brief carrying the line. Closing that needs a `SubagentStart` check -- unfiled, and it must be silent when a venv is present or it becomes noise every reader learns to skip. **Not** done: `new.ps1` calling `git worktree lock` while a session is live. |
| B11 | **Started** | Rule 3d closes the worst of G9: `git worktree remove` / `move` on another session's checkout. **Not** done: the rest of the absent verbs (`rm`, `mv`, `sparse-checkout`, `checkout-index`, `bisect`, `branch -f`, `update-ref`, `read-tree`), `gh pr checkout`, and G11's teardown half — `prune-merged.ps1` is still sibling-only. Its wrong-cwd green no-op is **fixed** (2026-07-30): it refuses with exit 2 and names both paths, and it will not `--force`-remove a sibling containing a nested worktree. **Raised in priority 2026-09-16:** B6 now documents nested worktrees as where build sessions land, so their missing teardown is no longer incidental -- it is the default population. Measured that day: **86 nested worktrees**, and a venv is ~887 MB, so roughly 42 GB has already accumulated with no scripted way to reap it. Disk is the lesser half. `ensure-venv.ps1` decides freshness by file existence, so a nested tree that lives for weeks keeps a venv pinned to an old `constraints.lock` while the script correctly reports nothing to do -- the ruff 0.16 drift its own comment describes, now aimed at the default population. The fix is small and known: teach `remove.ps1` to resolve a nested name, and leave `prune-merged.ps1`'s blanket nested exclusion alone. A removal guard used in the field that day, and the shape this should adopt: head equals `origin/<branch>` **and** `git status --porcelain` empty **and** `git ls-files --others --exclude-standard` empty -- all three, per worktree, before anything is touched. A committed-and-pushed worktree is fully recoverable and an uncommitted edit is not, which is the only line that matters. Five siblings were removed under it and one held back as dirty and unpushed. |

Remaining order: the rest of B11 (its teardown half above), the rest of B6, then the B1/B3/B4 remainders.
