...
What survives is a composite, not a count. The composite draws on four things no single figure captures: deterministic enforcement, a written threat model, independent external verification, and evidence-backed honest claims. Three are required; independent external verification is desired (signal 10). It plays out in three layers. First, deterministic blocking enforcement: SAST, SCA, and secret-scan gates that fail on any new finding from a clean baseline, verified in the workflow file rather than the tool list. Second, the structural controls: a written per-interface threat model, supply-chain integrity, fail-closed defaults, interface authentication with TLS everywhere, and a tamper-evident audit log. Third, the controls a scoreboard can never supply: independent external verification and evidence-backed honest claims. Independent external verification is a desired state, not a requirement (signal 10); its absence is recorded but does not cap the grade. A green CI checkmark from a gate you ran on yourself does not substitute for an adversary who did not.
...
> **The composite certifies "secure-enough," never a single row.** The verdict is the composite defined in the Executive summary. A perfect durable layer with conflicting in-tree scorecards (signal 11) is not an A. A missing external challenge (signal 10) does not withhold an A. Signal 10 is a desired state, not a requirement, and the scorecard still records it as Absent. *(Changed 2026-09-24 by owner ruling. This said a perfect durable layer "with no external challenge (signal 10) and conflicting in-tree scorecards (signal 11) is not an A". It also said "The process layer is not optional weight".)* The rest of the process layer (signals 9, 11 and 12) is not optional weight. No green scanner count, ASVS %-pass, or self-assessment substitutes for the rows a solo project finds hardest to supply. This is the security-side analogue of the [Code Quality rubric's](Code_Quality_Standards.md) "structure over scoreboards."
...
| 10 | Independent external verification | SDS §6.3 | Not an ASVS requirement: ASVS 5.0 L3 does not require independent review. *(Corrected 2026-09-23. This cell said "ASVS L3 verification mandate", which was false.)* | — | Owned by SDS §6.3; surfaced here as a desired state, not a cap on the grade |
