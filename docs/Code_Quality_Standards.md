...
| 10 | Lint breadth | **Built** — PR \#1047 | `[tool.ruff.lint] extend-select = ["B","C4","SIM","UP","I","RUF006"]` (RUF006, the unreferenced-task rule, added 2026-09-25 under BACKLOG \#1093 with its one finding fixed), enforced by the required `ruff check` leg. B008 (FastAPI DI, ~460 hits) handled via `extend-immutable-calls` + a route-layer per-file ignore (real `x=list()` bugs still caught); **515 violations auto-fixed** (import sort, pyupgrade, safe simplify); **235 non-auto-fixable grandfathered** with per-line `# noqa` → clean baseline, new code must comply |
| 11 | Complexity triage | **Built (advisory)** — PR \#1028, **sharpened 2026-07-27 (v0.10)** | `quality-advisory.yml` runs `ruff --select C901 --exit-zero` (advisory, never gates), **plus a merge-base-vs-HEAD delta** (`scripts/quality/c901_delta.py`) that reports only functions a PR *introduced* or *made worse*. **Re-measured 2026-08-10: 132 functions exceed** `C901`**\>10** across 46 files (122/43 on 2026-07-27; 85 on 2026-07-13) — instrument `ruff check --select C901 --exit-zero`, scanning `messagefoundry/` only. **Widened to the whole repo 2026-09-25 (BACKLOG #1093):** 170 findings in `messagefoundry/`, 185 with `messagefoundry_webconsole/`, 278 across the repo (ruff 0.15.22). The raw list is unusable as a diff signal — every finding anchors on a single `def` line — which is what the delta exists to fix |
...
| Signal 11 — complexity | `ruff check --select C901 --exit-zero "$C901_SCOPE"`, the scope stated once as `.` in `quality-advisory.yml` (BACKLOG #1093; it was the literal `messagefoundry` until 2026-09-25) | The whole repo, less at least pyproject's `[tool.ruff] extend-exclude`, everything `.gitignore` lists, and ruff's default excludes | Advisory only: it reports and never gates. `tests/test_quality_advisory_invariants.py` pins the scope |
