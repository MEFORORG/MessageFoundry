...
## Amendment 2026-07-02 — the wake-or-backstop contract (WS-C empty-claim storm)

**Context.** The 1,500-lane bench campaign (WS-C) found that with wake events ON but the 0.25 s
`poll_interval` backstop unchanged, the O(lanes × stages) idle workers still re-claim the shared
`queue` every poll — an **empty-claim storm** that saturated the store box (~92% CPU, `LCK_M_U`
convoy) at **zero** message volume, inert to `poll_interval` and `pool_size` tuning. B12's original
"leaves poll_interval at 0.25 s in both arms" resolution (item 4 above) is therefore superseded:
per-lane wake must also *retire the short idle poll* to deliver its idle economy.

**Decision.** With `per_lane_wake = true`, `_wait_for_work` backs the idle backstop off to
`_PER_LANE_IDLE_BACKSTOP_SECONDS` (30 s). The wake is the latency path; the backstop is only a
lost-wake safety net. This is safe because **every deferred-work source now has its own wake**:

1. a producer commit wakes its lane (this ADR, unchanged);
2. a **`mark_failed` reschedule arms a one-shot per-lane timer at the row's `next_attempt_at`**
   (+50 ms slack so the claim's due-time predicate is already true) — `mark_failed` now returns the
   reschedule time (`float | None`; `None` = dead-lettered/missing → no timer). Applied on the
   delivery worker's live-continue retry paths; the STOP path returns (a reload re-arms the lane);
3. startup/promotion recovery (`reset_stale_inflight` / `recover_inflight_on_promotion`) runs
   **before** workers spawn, and a worker's first loop iteration always claims;
4. the clustered lease-reclaim sweep calls `notify_work` (`LeaderMaintenanceRunner(on_reclaimed=…)`)
   when it re-pends rows mid-run.

**At-least-once is preserved:** workers always re-claim at the top of the loop, so a genuinely lost
wake (or a timer lost to shutdown/reload) costs at most one backstop interval of latency — never a
lost message. `per_lane_wake = false` keeps `poll_interval` byte-identical (pre-B12 behavior).

**Consequence / limits.** This kills the **idle** storm only (bench-validated: store waits → ~0 at
rest). The **loaded** claim convoy at high lane counts persists and is addressed separately by the
shared per-stage claimer (the WS-C Phase-1 design). The `per_lane_wake` default stays `false`; a
default flip is decided together with the Phase-1 work.
