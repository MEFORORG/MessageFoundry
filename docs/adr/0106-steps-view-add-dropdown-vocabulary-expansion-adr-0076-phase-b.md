...
| Send | `sends.append(Send("OB_ACME_ADT", msg))` + managed `sends = []`/`return sends` scaffold | native (`insert_send` op — **superseded by [ADR 0108](0108-steps-view-accumulator-send-fan-out-copy-on-send-authoring.md)**; this ADR shipped `return Send(...)`, ADR 0108 repointed it to the accumulator fan-out) | destination = outbound picker; inject `Send` import; ＋ dest / Add destination to fan out | send (`appended: true`) |
...
- **(A) Multi-line statement templates through the audited paste machinery.** Generalize `_apply_insert_row` on a `template` selector; If/For Each/Filter/Raise render to source and route through `_parse_pasted_block` → reindent → per-line length guard → keepends splice → `_assert_reparses`. (**Send** later moved to its own `insert_send` accumulator op — [ADR 0108](0108-steps-view-accumulator-send-fan-out-copy-on-send-authoring.md) — so it no longer rides this template path.) Reuses the already-verified paste helpers, so validity + `ruff`-format cleanliness come for free; single-line vocabulary calls keep the fast path.
