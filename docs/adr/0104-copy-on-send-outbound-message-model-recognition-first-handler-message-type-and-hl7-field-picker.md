...
**Status:** Accepted (2026-07-13) — owner-ratified. Q1 copy-on-Send + Q2 `message_type_of` built (PR #995, BACKLOG #230); Q3 §2.3 HL7 field picker building. The §2.3 adoption gate is satisfied by the owner's Steps-view endorsement; the five §3 spec blockers are closed by the build; the copy-on-Send **default-flip** is the one deferred follow-up (gated on an estate scan + throughput benchmark).
...
> Ratified. Every item below is resolved by the build **except** the copy-on-Send **default-flip**, which stays a deferred follow-up (BACKLOG #230, gated on an estate AST scan + a throughput/pickle benchmark).
...
- [x] **Snapshot implementation** — copy-on-write vs unconditional structural snapshot: decide against the throughput benchmark; confirm the default-flip trigger for copy-on-Send after the estate scan.
