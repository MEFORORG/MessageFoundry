...
5. **Execution-path matrix.** The snapshot invariant and the enforcement predicate must hold on **both** the split path (`dryrun.py` `transform_one`) **and** the fused inline fast-path (ADR 0057), and across the **subprocess sandbox** IPC boundary (ADR 0087) — see §4.
...
- **AC-4** — WHEN a fan-out `Send.message` is marshalled across the subprocess-sandbox pipe (ADR 0087), THE SYSTEM SHALL round-trip it through the IPC codec and re-encode it in the parent without error. (Re-worded: the pipe no longer pickles — see ADR 0087's MFW2 amendment.)
  → `tests/test_copy_on_send.py::test_snapshotted_send_message_survives_the_sandbox_codec`,
  `tests/test_sandbox_codec.py::test_copy_on_send_rebuild_is_a_provable_no_op`
...
## 8. Resolved on acceptance (2026-07-13 — owner-ratified)

> Ratified. Every item below is resolved by the build. The copy-on-Send **default-flip** (BACKLOG #230), the one item that stayed a deferred follow-up, is now **resolved too** — see **§8.1**.

- [x] **Match grammar** — confirm the one-line spec for `message_type_of` / declaration: code-only (`"ADT"`), exact (`"ADT^A01"`), list, wildcard-as-sugar — so picker, resolver, and predicate agree (component-wise throughout).
- [x] **Snapshot implementation** — copy-on-write vs unconditional structural snapshot: **decided — genuine copy-on-write** (`Message.copy()` shares the tree; a mutator deepcopies-on-write). The default-flip trigger (estate scan + benchmark) is **satisfied** — copy-on-Send is now **default-ON**. See **§8.1**.
- [x] **Picker MVP breadth** — recognition-inferred type + generic catalog only first, or include sample-MSH-9 scoping (lean: defer sample scoping for a smaller, PHI-free MVP).
- [x] **Non-HL7 fan-out** — string-building + `Send(to, str)` now (recommended) vs freezing `RawMessage.raw` / a builder later.
- [x] **Value-side assist** — HL7-table values for coded fields (PID-8 etc.) is out of Q3's path-picker scope; schedule as a separate sibling item if wanted.
- [x] **BACKLOG number** — allocate a BACKLOG item for the build (`pwsh -NoProfile -File scripts\coord\alloc.ps1 -Kind backlog -Title "…"`) and link it here before flipping to Accepted.
- [x] **Confirm the two verified corrections** are honored in the build: no reliance on a `Message.parse` `.strip()` mechanism (§2.1); inbound batch split is `parsing/split.py`+`file.py`, not ADR 0082 (§3).

### 8.1 Default-flip resolution — copy-on-Send is now default-ON (2026-07-14, BACKLOG #230)

The default-flip was gated on **(a)** an estate AST scan for the one regression surface — *"construct a `Send`, then mutate the message it referenced before return"* — and **(b)** a throughput benchmark. Both ran:

- **Estate scan (compatibility) + triage.** A conservative linear-DFS scan (no aliasing analysis) of the real estate flagged **exactly 1** handler constructing a `Send` and then mutating a message before return; in-repo samples flagged 0/11. **The deferred human eyeball is now done** (2026-07-14): that lone handler mutates an **independent clone** — a fresh re-parse it takes precisely so its second `Send` carries a separately-stamped copy — *not* the object the first `Send` captured. It is a **false positive** of the loose heuristic, not a divergence. So genuine *"construct a `Send`, then mutate the **sent** object before return"* divergence in the estate is **0** — the flip changes delivered bytes for **zero** existing handlers, and is pure safety margin for the helper/loop-laundered aliasing forms a static lint cannot classify.
- **Cost + the copy-on-write fix.** As first shipped, the snapshot was an **unconditional eager** `Message(copy.deepcopy(self._m))` — measured **~5 µs (small ADT) to ~27 µs (24-OBX ORU) per Send**, paid on *every* delivery regardless of divergence (contradicting this ADR's "zero-copy common path" premise). Fixed by making `Message.copy()` **genuine copy-on-write**: the clone shares the backing tree until either side is mutated, at which point the mutator (`_ensure_owned`) takes the private deepcopy. Result: `copy()` drops to **~0.1 µs** (57× cheaper) and a deepcopy fires **only on an actual divergence**. The common single-Send / no-post-mutation path is now truly zero-copy.

**Decision:** flip `[pipeline].snapshot_on_send` to **default `True`** on the copy-on-write base. The one honest behaviour change stands — a handler that constructs a `Send` then mutates the same message before returning now delivers the per-destination snapshot (the intended fix) instead of the last write. Set `[pipeline].snapshot_on_send = false` to restore the pre-ADR-0104 behaviour. (The runtime ContextVar / `dry_run` param defaults stay `False` — a `Send` built outside an active transform run, or a preview that doesn't opt in, still does not snapshot.)
