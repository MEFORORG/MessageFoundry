...
- **Status:** **Accepted (2026-06-15)** — ratified on the owner's "go"; the open questions below are resolved. **Built.**
...
## Amendment (2026-07-14) — body-secret substitution (BACKLOG #236)

WS-Security carries credentials in the `<Header>` (`_build_username_token`, [`soap.py`](../../messagefoundry/transports/soap.py)). Some registries — a CDC-IIS-style `submitSingleMessage` — instead take username/password/facility as **elements of the operation `<Body>`**. `ws_password` cannot reach the body, the Handler's `<Body>` fragment is spliced verbatim (no templating — §1), and a Handler cannot resolve `env()` at call time. So the only pre-existing way a credential could reach the body was to **put it into the message the Handler returns** — and that message is persisted (outbound / done / dead-letter rows), replayed, and rendered in operator views, on a store that is **unencrypted by default**. **The core rule of this amendment: the secret must never enter the message.**

**Contract.**

- `Soap()` gains `body_secrets={placeholder_token: env(secret)}`. The Handler emits the opaque `placeholder_token` inside its `<Body>` (staying **pure** — it never holds the credential); the transport substitutes the `env()`-resolved secret in `send()`, **after** the payload leaves the store and (WS-\* mode) is wrapped by §2, and **before** `_post` encodes + signs it. The wire (and any ADR 0018 detached JWS over those bytes) carries the secret; the store holds only the token. Absent `body_secrets`, `send()` is **byte-identical**.
- **This extends §1's value-placement table:** a *config* secret is now placed **inside** the `<Body>`, at a declared token, substituted post-store. It does **not** relax §1's Handler-purity rule — the Handler still authors only a token, never the credential — and it is the first case in which the transport **mutates** the Handler's `<Body>` (previously the transport touched only the `<Header>`).
- **Flat-key hoist (not a nested map).** The factory desugars `body_secrets` into top-level `body_secret_tokens: list[str]` + `body_secret_value_<i>: EnvRef`, because `resolve_env_settings` resolves only **top-level** `EnvRef`s (a nested one is copied verbatim — the tested `_reject_envref_odbc_params` ruling); a nested `{token: env()}` would reach the connector as `EnvRef` **objects** spliced onto the wire as their `repr`. The flat keys also arm the ADR 0050 missing-value gate and are redacted by `_is_secret_setting`'s `body_secret_value_` prefix (the paired `body_secret_tokens` are **not** secret — a placeholder is public in the committed Handler source).
- **Code-first only, fail loud.** Each value must be an `env()` ref with **no `default=`** (a fallback secret would leak via `display_settings` → `graph --json`) and no `cast=`. A `connections.toml` `[settings.body_secrets]` table arrives as raw `{"env": …}` dicts (`parse_env_setting` is top-level only), so the `EnvRef` check refuses it pointing back to code-first — there is **no TOML/GUI round-trip** (which would either persist a plaintext secret or corrupt the connection on save).
- **Escaping.** The substituted secret is escaped for **element and attribute** contexts (`& < > " '`) — `saxutils.escape` does only `& < >`, and the §2a well-formedness gate accepts a token in an attribute position and runs *before* substitution, so an unescaped quote could break out of the attribute.

**Accepted residuals (a soft boundary, written down deliberately).**

1. **The placeholder is not a structural seal.** The occurrence check is content-blind `payload.count(token) == 1`; inbound HL7 is attacker-influenceable (CLAUDE.md §5). A 16–64-char high-entropy token, a substring-disjointness rule, and a **fail-closed exactly-once** check (0 or ≥2 → a **permanent** dead-letter naming only the token + count, **nothing POSTed**) make a collision infeasible in practice — mitigations, not a proof. Only a no-token design (the transport composes the credential elements itself) is structurally immune, and that needs a SOAP 1.1 envelope generator that does not exist and cannot be committed to against an unread WSDL.
2. **`capture_response` scrubbing is defence in depth, not a seal.** A validating registry may echo a bad credential in a 200-OK `<Fault>` that `capture_response` persists; the transport reverse-maps the secret (raw + escaped) to `***` for secrets ≥ 8 chars (a short facility code a CDC-IIS ACK legitimately echoes must survive). The match is **literal**, so a secret the partner echoes in any transformed form passes through unmasked — treat a captured reply as potentially secret-bearing regardless, which is the operator-facing consequence and holds whatever the transformation was. *(Wording narrowed 2026-08-14: this residual previously enumerated specific transformations. **The decision is unchanged** — the residual is accepted exactly as before; only the recipe is gone, because a reader learns the limitation from the class statement while the enumeration handed over a ready-made list to try. The docstring at `transports/soap.py` was narrowed in the same change, and the two must stay in step: narrowing either alone leaves the sentence published in the other.)* `reingress_to` is **refused** with `body_secrets` (it would promote the scrubbed reply into a first-class message).
3. **A rejected credential does not dead-letter.** With `capture_response=True` a `<Fault>` inside a 2xx is recorded as `outcome="rejected"` on a **DONE** row and the message finalizes `PROCESSED` — operators reconcile from the captured response, not a dead-letter.

**Pre-check that may moot this (§4 of the handoff).** If the live WSDL accepts a WS-Security `UsernameToken` **header**, use `ws_security=True` + `ws_username`/`ws_password=env(…)` (already built) — no `body_secrets` needed. The shipped [`samples/config/IB_IMMUNIZATION_VXU.py`](../../samples/config/IB_IMMUNIZATION_VXU.py) assumes exactly that header form of the same operation.

**Declined alternatives (this amendment).**

| Alternative | Why rejected |
| --- | --- |
| **Handler builds the body with the real password** (give it a send-time resolver) | The credential would then be **inside the persisted message** — the store, replay, dead-letters, operator views. The whole point is that it must not be. **Fatal.** |
| **Nested `body_secrets={token: env()}` reaching the connector unresolved** | `resolve_env_settings` is top-level only; the connector would splice `EnvRef` *reprs* onto the wire. Hence the flat-key hoist. |
| **`connections.toml` / GUI support** | The nested table has no lossless flat round-trip (the writer emits the hoisted keys, which the factory would then reject), and the VS Code connection editor renders a nested map as `[object Object]`. Refused loudly instead of corrupting silently — code-first only. |
| **No-token "compose the credential elements" design** | Needs a SOAP 1.1 envelope generator the engine does not have, cannot express a `<Credentials>` wrapper / interleaved `xs:sequence`, and removes plain mode. Not buildable against an unread WSDL. |
