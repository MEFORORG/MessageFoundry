...
2. **`capture_response` scrubbing is defence in depth, not a seal.** A validating registry may echo a bad credential in a 200-OK `<Fault>` that `capture_response` persists; the transport reverse-maps the secret (raw + escaped) to `***` for secrets ≥ 8 chars (a short facility code a CDC-IIS ACK legitimately echoes must survive). A base64'd / hashed / re-encoded echo defeats it. `reingress_to` is **refused** with `body_secrets` (it would promote the scrubbed reply into a first-class message).
