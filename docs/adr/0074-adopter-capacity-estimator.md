...
**B14 — Downstream artifacts carry the stale text and would ship the gate holes even if only this ADR is amended.** — ✅ **RESOLVED in this change: all four corrected** (see item 13 below).
...
13. ~~**Amend the downstream artifacts** in the same change, or the gate holes ship regardless of what this ADR says.~~ ✅ **DONE (2026-07-14) — all four amended alongside this ADR:**
    - [`docs/adr/README.md`](README.md) — the 0074 status row now carries the build gate.
    - [`docs/testing/FEATURE-COVERAGE-PLAN.md`](../testing/FEATURE-COVERAGE-PLAN.md) — **SCALE-16** gated, and its *"build **AC-4** first — a correctness/PHI-safety assertion independent of any live number"* recommendation is **retracted**: AC-4 is circular (**B6**). The guard set is restated as AC-1/AC-3/AC-5/AC-6(+`INCONCLUSIVE`).
