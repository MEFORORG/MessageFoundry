...
**Status:** Accepted (2026-07-07) — owner ratified. ⛔ **BUILD GATED (2026-07-14)** — a validity re-check against the STEP-4 Arm-0 findings found the ADR's **premise and hard requirements still hold**, but its **measurement method is materially stale and one part is unsafe to productize as written**: the "no-loss reconcile" it names as *the only success gate* is the very gate Arm 0 proved **over-reports** (a filling rung passes it while E2E latency runs away). **Do not build the measurement layer until the owner re-ratifies the revised gate + estimand** — see the [Amendment](#amendment-2026-07-14--validity-re-check-vs-step-4-arm-0-build-gated) below. (The fail-closed **guard** layer — AC-1/3/5/6 — is unaffected and remains buildable.) ✅ **RE-RATIFIED (2026-09-14) — the owner re-ratification this gate waits on HAS NOW HAPPENED, so the sentence immediately above is superseded and is kept only as the record of the gate it set.** All three corrections are adopted: **sustain means no-loss AND non-filling**; **the estimand is DELIVERY**; **the aggregate is `min(measured concurrent, Σ per-interface)`** — required changes 1, 3 and 4 below. The **rules** of the measurement layer are therefore settled. **The measurement BUILD itself is not thereby authorised** — it stays demand-gated on BACKLOG #96's own trigger — and the guard layer (AC-1/3/5/6) may start. See the [Re-ratification](#re-ratification-2026-09-14--owner) below.
...
The step-reading rule is "the highest step where **achieved ≈ offered**", and `achieved = acked / rec.wall_seconds` (`harness/load/report.py:333, :361`) — an **ACK** rate. `_is_ceiling`'s second term is `achieved_intake < offered * (1 - _INTAKE_TOL)` (`shardcert.py:1061`), explicitly *"Deliberately **not** `delivered < offered`"*. Arm 0 retracted **"engine sustains 26"** precisely because 26/s was ingress acceptance against ~16 msg/s delivery. Per [ADR 0101](0101-pre-registered-falsifier-discipline-for-performance-measurement.md), the reported figure MUST state its estimand or be quoted in **deliveries/s**. This defect lives in the ADR's *named* path — it is not fixable by switching paths.
...
13. ~~**Amend the downstream artifacts** in the same change, or the gate holes ship regardless of what this ADR says.~~ ✅ **DONE — all four amended** (the first three 2026-07-14; `docs/BACKLOG.md` #96 in this change):
    - [`docs/adr/README.md`](README.md) — the 0074 status row now carries the build gate.
    - `docs/testing/FEATURE-COVERAGE-PLAN.md` — **SCALE-16** gated, and its *"build **AC-4** first — a correctness/PHI-safety assertion independent of any live number"* recommendation is **retracted**: AC-4 is circular (**B6**). The guard set is restated as AC-1/AC-3/AC-5/AC-6(+`INCONCLUSIVE`).
    - [`docs/THROUGHPUT.md`](../THROUGHPUT.md) §7 — the *"aggregate = the sum of the interfaces"* sizing rule is flagged **measured-false (~11× over-report)**; take `min(measured concurrent aggregate, Σ per-interface)`.
    - [`docs/BACKLOG.md`](../BACKLOG.md) **#96** — a **BUILD GATED (measurement layer)** advisory added, with the *quick-win / Difficulty-5* score flagged as no longer trustworthy (**B7**: v1 must be re-priced); the *sum-across-interfaces* rule corrected; and the two unsafe guidance lines fixed — the *"only trustworthy success gate"* now requires the **non-filling** companion, and the **poller-zero remedy is marked circular**. Note **#96 already named the right signal** — *"`in_pipeline` trajectory (flat vs climbing) is the clearest pass/fail"* — and **this ADR dropped it** in favour of the loss gate alone. That regression is the root of **B1**.
...
### Re-ratification (2026-09-14) — owner

> **The build gate of 2026-07-14 asked for one thing from the owner: re-ratify the revised gate + estimand. That has now happened, and this section records it so nobody re-derives it or re-opens it.** Nothing above is deleted. The Amendment's blockers, its corrections and its fourteen required changes all stand as written; what changes is that three of them now carry the owner's ratification rather than waiting for it.

**Adopted, verbatim in substance:**

1. **Sustain means no-loss AND non-filling.** A rung that is lossless and eventually drains but was filling through the hold is **not** sustained. (Required change **1**; the AC-7 that required change **12** asks for is the acceptance criterion this ratifies.)
2. **The estimand is DELIVERY**, not intake acceptance. (Required change **3**, closing blocker **B3**.)
3. **The aggregate is `min(measured concurrent, Σ per-interface)`** — measured, never composed. (Required change **4**, closing blocker **B4**. §Decision item 1's *"the sum across interfaces"*, and the same rule at `docs/THROUGHPUT.md:260`, are **superseded by this** and are kept only as the text the correction is against.)

**WHAT THIS UNBLOCKS, AND WHAT IT DOES NOT — the distinction is the whole point of writing it down.**

- **Unblocked: the RULES of the measurement layer.** The gate's stated condition is met, so no later pass should read the ⛔ banner as still waiting on the owner.
- **NOT unblocked: the measurement BUILD.** It stays **demand-gated** on BACKLOG #96's own trigger — an adopter needing a pre-cutover capacity check on their own hardware, or BACKLOG #93 needing a per-deployment baseline to calibrate against. Neither has fired. **A ratified rule is not a funded build.**
- **The fail-closed GUARD layer (AC-1 / AC-3 / AC-5 / AC-6) MAY START**, as the Amendment always said it could, and it is independent of any live number.
- **The other eleven required changes are untouched by this.** They are engineering work on this ADR's method, not owner forks, and ratifying three of them settles nothing about the rest.

**No new ADR number was allocated, deliberately.** These are corrections to ADR 0074's own method, already enumerated in its own Amendment, so they belong here rather than in a sibling that would have to be read alongside it. Recorded in the same change: this section, the Status line above, the index row at [`README.md`](README.md), and BACKLOG **#96**.
