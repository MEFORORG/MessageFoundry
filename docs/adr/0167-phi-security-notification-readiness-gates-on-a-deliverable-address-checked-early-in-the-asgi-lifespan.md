...
# ADR 0167 — PHI security-notification readiness gates on a deliverable address, checked in the ASGI lifespan after the bootstrap administrator is created

> **THE FILENAME IS HISTORICAL. THE HEADING ABOVE IS NOT.** This file is still named
> `0167-...-checked-early-in-the-asgi-lifespan.md` because it was allocated under the EARLY-LIFESPAN
> placement that this ADR's own overturn banner then ruled **impossible**. It is deliberately **not**
> renamed: the name is an address, every existing citation resolves through it, and a rename would
> break them to fix a word. The heading names the placement that **shipped** —
> `_assert_security_notice_is_deliverable` runs after `bootstrap = await auth.initialize()`, which is
> the **LIFESPAN (post-bootstrap)** row of the placement table below. **Read the filename as an
> address, never as a claim** (BACKLOG #1020). Whether the file should eventually be renamed is a
> judgement about the record that nobody has ruled on, so it is left open rather than taken here.

- **Status:** Accepted (2026-09-09) — the gate is BUILT and wired, in the form the overturn banner below settled and NOT the one this ADR's FILENAME names (the heading above has been corrected to the shipped placement). Build state corrected 2026-09-09; see the bullet beneath
- **Build state corrected 2026-09-09.** The status line above previously read *"Proposed (2026-08-15) — the predicate is built (`29a026e2`); the gate that consumes it is not yet written."* **That was true when written and is false now, in the UNDER-claiming direction** — a reader planning off it plans to build something that already exists — so it is recorded here rather than deleted. **The parenthetical is the date this record was corrected.** Nothing records a ratification on 2026-08-15, which is the day the ADR was proposed; the code landed earlier, at `4a8f062d6` (2026-08-19, PR #394), whose parent does not carry the symbol. Re-verified against the tree by symbol, not by line number: `_assert_security_notice_is_deliverable` in [`api/app.py`](../../messagefoundry/api/app.py) runs inside `create_managed_app`'s `lifespan`, after `bootstrap = await auth.initialize()` and therefore AFTER `engine.start()`. That is the **LIFESPAN (post-bootstrap)** row of the table below — the placement the overturn banner said this check would have to take, and **NOT** the EARLY-LIFESPAN placement this ADR's FILENAME still asserts. (The heading was corrected to the shipped placement on 2026-09-15, BACKLOG #1020; the filename was deliberately left alone so existing citations keep resolving. This sentence used to read "title and filename", and it is corrected in place rather than rewritten, because a reader who absorbed the old wording needs to see which half moved.) Under its three preconditions (auth notices on, `[alerts].security_notifications_required` true, derived data class PHI) it raises at `enforce` and logs a warning otherwise; `tests/test_security_notice_deliverability.py` carries the wiring test (`test_the_LIFESPAN_refuses_and_not_merely_the_predicate`) with a warn-side positive control beside it. **The shipped gate does NOT call `has_notifiable_admin()`** — it inlines the same enumeration (`store.list_users()`, the disabled-skip, the `notify_email` test, the `Role.ADMINISTRATOR` test), so the predicate has **no non-test call site**, and the open-coded copies of "who is an enabled administrator" now number **four**: the three in `auth/service.py` that Consequences records, plus the gate's own in `api/app.py`. **TWO THINGS THIS ADR PROMISED WERE OWED UNTIL 2026-09-15 AND ARE NOW WRITTEN (BACKLOG #1020).** Recorded as a correction rather than swapped out, so a reader who absorbed the owed-list can tell which entries moved. (1) `docs/DEPLOYMENT.md` carried only its two exit-2 statements and now carries the exit-3 line accepted below, as *"Exit codes -- a startup-stage refusal exits 3, not 2"* under its bind-guard section. (2) `tests/test_notice_gate_refusal_terminates_under_uvicorn.py` arranges this gate's real preconditions, runs the app under uvicorn in a subprocess, and asserts the process exits **3** with the gate's own refusal text in its output, against a warn-arm control that reaches a RUNNING server and self-stops with 99. That closes #1020's rider and the re-measurement demanded under *"What is NOT demonstrated"* below. **Neither of the two tests standing near it discharges either, which is why a third was written rather than one of them widened:** `tests/test_lifespan_startup_unwinds.py` runs uvicorn in a subprocess but PATCHES this gate's call site with a raising stub, deliberately, because its subject is BACKLOG #1257's unwinding and it must not fail when the gate that happens to sit there changes; and the wiring test above drives the real refusal under `lifespan_context`, where there is no process to exit. **STILL OPEN and not closed here:** `has_notifiable_admin` has no production call site, because the gate inlines the enumeration, so the four open-coded copies of "who is an enabled administrator" recorded above are unchanged
- **Date:** 2026-08-15
...
> ## DISCHARGED 2026-09-15 (BACKLOG #1020). BOTH ITEMS BELOW ARE NOW MEASURED ON THE REAL GATE.
>
> `tests/test_notice_gate_refusal_terminates_under_uvicorn.py` arranges this gate's real
> preconditions -- notices on, `[alerts].security_notifications_required` true, `enforcement =
> enforce`, SMTP fully wired, and an empty store whose only account is the addressless bootstrap
> administrator the lifespan itself mints -- runs the app under `uvicorn` in a subprocess, and
> asserts the process **exits 3** with the gate's own refusal text in its output. So the real
> refusal terminates, in a lifespan that unwinds properly, at the exit code this ADR accepted.
>
> **The section is kept rather than deleted, because the reason it existed is the lesson.** It was
> written to stop this ADR being cited as discharging a rider that exists precisely because someone
> inferred. The obligation is met by a demonstration, which is the only thing that could meet it.
>
> **What the discharge does NOT cover:** the arms in the exit-code table above were taken on the
> minimal repro and are still repro measurements. The real gate is now measured at exit 3; nobody
> has re-run `sys.exit(2)` from inside the shipped lifespan, and nothing turns on it.
