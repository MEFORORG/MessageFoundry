...
> autocommit, so a write that got past its statement gate would commit. `fhir_lookup` sends only GET
> requests to the FHIR server, by structure. Its one other request is the SMART token POST, which
> carries no caller input. `db_lookup` runs a caller-supplied statement. The privilege of the account
> it dials bounds it, and its in-process layers are defence in depth only. See
> [ADR 0010](0010-handler-callable-db-lookup.md#consequences)'s correction under *Scope* and
