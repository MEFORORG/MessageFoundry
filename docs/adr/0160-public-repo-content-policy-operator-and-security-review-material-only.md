...
(`MEFORORG/MessageFoundry-vault`) and was committed there **before** anything left the public tree.
...
leaving the tracked tree, at `MEFORORG/MessageFoundry-vault` branch `lander/adr0160-custody`, verified
...
⛔ **The vault's CI is off.** `gh workflow list --all` on `MEFORORG/MessageFoundry-vault` reports every
