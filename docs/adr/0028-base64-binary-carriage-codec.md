...
- **Retrofit the ERROR-path latin-1 fallback — RESOLVED 2026-07-13 in this ADR (see the [Amendment](#amendment-2026-07-13--error-path-latin-1-retrofit-closed-ingest-4) below), NOT a dedicated ADR** (that was the open scope question — answered: amend in place). The stale `wiring_runner.py:736` single-site reference predated the split into two ingress handlers (`_handle_inbound` / `_handle_inbound_http`) each with two ERROR branches (binary **over-cap** + text **decode-error**); all **four** live latin-1 ERROR sites now route through a `_nul_safe_error_raw` seam that keeps the readable byte view when NUL-free and escalates to `mfb64:v1:` carriage (§3 `from_bytes`) only when a NUL is present. The fix also **extends past the ERROR path** to a post-decode NUL guard on the decoded-text/HL7 **happy** path (a lone NUL is valid UTF-8 and was riding the success path into the store). See the Amendment for the full scope, the accepted behavior change, and the deliberately-deferred store-layer/redaction hardening.
...
## Amendment (2026-07-13) — ERROR-path latin-1 retrofit CLOSED (INGEST-4)

The final *To resolve on acceptance* item ("Retrofit the … latin-1 fallback") is **RESOLVED in this ADR, amended in place** — not a dedicated ADR (that was the open scope question; answered here). It is closed by the **INGEST-4** fix (`messagefoundry/pipeline/wiring_runner.py` only).

**Scope was broader than the original follow-up framing.** The stale `wiring_runner.py:736` "lossless byte view" reference (one site, ERROR path only) predates the split into two ingress handlers — `_handle_inbound` (MLLP/file) and `_handle_inbound_http` (the HTTP listener) — each with two ERROR branches: a binary **over-cap** reject and a text **decode-error** fallback. There are **four** live latin-1 ERROR sites, now all fixed: `_handle_inbound_http` over-cap (~:2762) + decode-error (~:2790), and `_handle_inbound` over-cap (~:2986) + decode-error (~:3025).

**The seam.** A module-level helper `_nul_safe_error_raw(raw, content_type, *, text=None)` (~:252) returns a store-bindable `str`: the faithful, human-readable **latin-1/decoded view when NUL-free**, escalating to **`mfb64:v1:` byte-carriage (§3 `RawMessage.from_bytes`) only when a NUL is present** — so a NUL-free error raw stays readable, and the exact original bytes remain recoverable via `.raw_bytes`. U+0000 is the **only** store-hostile latin-1 codepoint (U+0001–U+00FF ride TEXT/NVARCHAR intact), and `b"\x00" in raw` ↔ `"\x00" in raw.decode("latin-1")` is bijective, so the NUL check on the view is exact. (This is an *ingress policy* — readable-when-safe, base64-when-NUL — so it lives in the pipeline, not in the pure `parsing/binary.py` codec.)

**The fix EXTENDS beyond the ERROR path.** The original follow-up worried only about an *accidental* non-UTF-8 body on the ERROR path. But a NUL is **valid UTF-8**, so it also rides the **success path**: a lone NUL inside an HL7 field value or a JSON body decodes cleanly and was being bound straight into the store (as `raw`, and on the HL7 path into `control_id`/`summary`). A fix touching only the four latin-1 sites would leave that wide open. So the fix adds a **post-decode NUL guard** in both `_handle_inbound` (~:3048) and `_handle_inbound_http` (~:2798), placed **before `Peek.parse` and any store write**, that dead-letters a decoded-clean NUL-bearing body. This single chokepoint transitively makes `text` — and everything derived from it — NUL-free for the rest of each handler.

**Accepted behavior change (RULED IN).** A NUL-bearing **text/HL7** body — which today *routes* on SQLite (Python round-trips the NUL) and on any **encrypted** store (the `mfenc:` base64 envelope is NUL-free), *SQL-side-truncates* on SQLite/SQL Server (`LENGTH`/`LIKE`/`substr` cut at the first NUL), and *crashes the connection* on Postgres — is now uniformly recorded **`ERROR`** and **NAK'd `AR`** ("invalid NUL in body", decode phase) on **every backend** (HTTP returns its 202-without-id receipt, no HL7 ACK; *amended 2026-09-26: HTTP now answers `422` here, by owner ruling, see the [ADR 0154 amendment](0154-synchronous-captured-downstream-reply-and-intake-authentication-for-the-inbound-http-listener-adr-0023-deferred-tail.md#amendment-2026-09-26-a-body-refused-at-ingress-is-answered-422-on-both-paths)*). This is a correct tightening, not a regression: current behavior is backend/encryption-dependent and incorrect, NUL is non-conformant in HL7 v2 field data / JSON / XML 1.0 / X12 / plain text alike, the exact bytes are preserved (recoverable via `RawMessage.raw_bytes`), and an **intentional** NUL-bearing payload keeps the sanctioned `content_type=binary` path (which already goes through `from_bytes`, untouched here).

**Invariant restored ([CLAUDE.md](../../CLAUDE.md) §2 count-and-log).** The severe cell was **PostgreSQL (asyncpg) + store-encryption OFF (the default: `encryption_key=None` / `require_encryption=False` → `IdentityCipher`) + a NUL-bearing body**: `record_received` raised `asyncpg.exceptions.DataError` (SQLSTATE **22021**) at bind, **uncaught**, unwinding out of `_handle_inbound` into `transports/mllp.py`'s `except Exception → break` — which logged and **dropped the whole TCP connection with NO ERROR row** (an accept-and-drop; it also dropped sibling frames in the same chunk, and a conformant re-send hit the identical deterministic error = a permanent invisible interface stall). The fix makes every ERROR row bindable, so the connection survives and the message is counted-and-logged.

**§3 contract reconciled.** §3 calls `RawMessage.from_bytes` "the **only** place bytes become a carried body," and §6 names the source-boundary `from_bytes` call site the "only seam." The ERROR-path helper and the NUL guard add **more `from_bytes` call sites, but all still WITHIN the same ingress source boundary** (`_handle_inbound`/`_handle_inbound_http`) — no consumer or codec hand-rolls base64, and the **one-encode property is preserved** (encoding stays centralized in `from_bytes`). Read §3's "one encode" as *one encode primitive, invoked only at the source boundary* — not *one textual call site*.

**Scope boundaries (deliberate).**

- The two **over-cap** sites (~:2762 / ~:2986) are effectively **dead code under default transport frame caps** (each transport defaults to the same 16 MiB cap and rejects a frame *before* the engine's `_INGRESS_MAX_BYTES` guard runs); they are reachable only if an operator disables/raises a transport cap. Fixed anyway (one line each, an engine invariant), but the live-severity framing is **"PostgreSQL + unencrypted store,"** not universal data-loss.
- On the other backends the pre-fix defect was **degradation, not a drop**: SQLite round-trips the NUL in Python but SQL-side length/`LIKE`/`substr` cut at the first NUL; SQL Server truncates the NVARCHAR; any encrypted store masks it entirely. No connection drop off Postgres.
- **Explicitly deferred, not shipped here** (named follow-ups, called out so nothing is over-claimed): a store-layer NUL assertion/sanitization in each backend's `record_received`/`enqueue_ingress` (would be a 3-backend surface, must run pre-cipher and **re-raise**, never swallow into a false AA ACK); stripping NUL inside `redaction.py::safe_text` (unreachable dead-defense once the guard fires before `Peek.parse`/strict-validation); and the separate polling-source (file/db/timer) poison-retry loop, which is a **different bug class** (mis-calibrated "assume-transient" backstops), not INGEST-4's connection-drop.

The regression tests land with the fix: SQLite/unit + HTTP-direct-call in `tests/test_nonhl7_ingress_size_cap.py`; the **load-bearing** gated cross-backend test in `tests/test_postgres_store.py` (only Postgres reproduces the drop — it asserts on `asyncpg` `DataError` / SQLSTATE 22021, **not** psycopg's "cannot contain NUL" string, which is the wrong driver for this stack) and its mirrored twin in `tests/test_sqlserver_store.py`; plus a PHI-no-log assertion on the persisted `messages.error` column.