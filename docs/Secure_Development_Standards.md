...
| Independent review | Third-party source-code review + penetration test per 800-115 — a **desired** state for the project, **not** a requirement and **not** an ASVS mandate. A production release needs either a current independent review or a dated risk acceptance in force (SDS-6.4.3). It covers the **OWASP ASVS 5.0 Level 3** verification, which is source-assisted/hands-on and may be performed internally; the external engagement adds independence + credibility. | Desired before a production release, after a major change, and periodically thereafter |
