...
- **SDS-3.8 — MUST confirm the instrument answers the question asked, not one adjacent to it.** *Evidence:* the question and what the instrument returns, written down side by side and checked to be the same sentence. The rules above
  ...
  **Re-reading caught none of the eleven; a check that could fail caught one immediately.** Nor were
  ...
  same sentence.
- **SDS-3.9 — A list that stands in for a surface MUST state how it was derived, or be worded as a floor.** *Evidence:* the derivation beside the list, naming the query, the corpus, the commit it ran at and the direction it searched from; the reviewer re-runs it at that commit and confirms it searched from the effect.
  SDS-3.6 prefers a floor to a list. This rule covers a list that is written anyway, most often the
  places where a gap still stands. Reading such a list cannot check it, because the missing entry is
  the one nobody wrote down. Re-running what produced it can. **The direction is the part that gets
  left out.** A search that starts from names ("every function whose name contains `delete`") and one
  that starts from the effect ("every call that issues a `DELETE`") return different sets. Only the
  second asks where the thing happens. An effect search is still a text search, so say why the token
  searched for is the only way the effect can happen, or say that it may not be. Re-running proves a
  list reproducible, and a name search re-runs cleanly while missing the same sites every time.
...
| Independent review | Third-party source-code review + penetration test per 800-115 — a **desired** state for the project, **not** a requirement and **not** an ASVS mandate (§6.3). It covers the **OWASP ASVS 5.0 Level 3** verification, which is source-assisted/hands-on and may be performed internally; the external engagement adds independence + credibility. | Desired before a production release, after a major change, and periodically thereafter |
...
| Version | Date | Change |
|---|---|---|
| 2.7 | September 24, 2026 | **Independent review is a desired state, not a requirement (owner ruling).** The §6.1 *Independent review* row called the third-party review + penetration test "the project's own pre-production gate (§6.3)", to be done "Before a production release". It now names it a desired state, not a requirement and not an ASVS mandate, and its timing is "Desired before a production release". SDS-6.4.3 is unchanged: a release still needs a current independent review or a dated risk acceptance. The companion [Secure Build Standards](Secure_Build_Standards.md) made the same change to signal 10. |
