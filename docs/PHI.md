...
| `queue.payload` (stage=`ingress`/`routed`, **permanently PENDING**) | **no purge reaches it.** The **DEAD** half of this tier is closed: a dead ingress/routed row now rides `[retention].dead_letter_days` on all three backends, because `replay` re-queues such a row from its own payload exactly as it does a dead outbound row. What is left is the row that never becomes dead and never drains. `purge_message_bodies` holds any message with a `pending`/`inflight` queue row ineligible — correctly, since at-least-once must not lose an undelivered body — so a row left permanently pending pins **both** its own payload and its message's `messages.raw`, indefinitely and with no window. That is reachable **by design, not by accident**: when an ingress row's owning inbound is absent from the registry, three sites re-pend it with `RetryPolicy(max_attempts=None)` — retry forever — rather than dead-letter it (`pipeline/wiring_runner.py:5057`, `:5296`, `:5480`), precisely so an ACKed-but-never-attempted message is never dropped for outliving a reload. There is no ingress-stage counterpart to the startup `dead_letter_missing_destinations` / `dead_letter_missing_handlers` sweeps, and adding one would collide head-on with that intent. So bounding this needs a decision this row does not pre-empt — an unreachable-lane rule has to tell "the operator is mid-reload" from "this lane is never coming back", and getting it wrong deletes a body the engine promised to keep. It stays listed here rather than being folded into the window above |
