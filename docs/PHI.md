...
1. **Application-level AES-256-GCM `[BUILT]`.** The store's `_cipher`
   ...
   **inside Vault/OpenBao Transit** and the data key never enters engine heap. Values carry a third
   ...
   Transit-backed audit MAC is threaded into the **SQLite** store only, so a `vault_transit` + SQL Server
   or Postgres deployment runs an **unkeyed SHA-256** audit chain (tamper-evident, not
   forgery-resistant).
...
- **No PHI in browser storage.** The only stored item is the session token, in an **HttpOnly + SameSite=
  ...
  `IndexedDB`, and no PHI is placed in a URL (so it cannot leak into history, bookmarks, or Referer).
...
| Tier | Status |
|---|---|
| `reference.value` | no purge — a snapshot is replaced only by the next sync's build-new-then-flip |
