...
| `[backup].destination/mefor-backup-*.mfbak` (ADR 0049 DR backup) | **Yes** — a consistent store snapshot (full inbound + outbound bodies) + the config bundle | **Yes** — AES-256-GCM `.mfbak` chunked-AEAD codec under the **store DEK** (`resolve_active_key`); an identity-cipher (no-key) box is **refused** unless `[backup].allow_unencrypted` writes a `.mfbak.plain` | A **second** at-rest PHI copy at the local/UNC destination; bounded by keep-N retention + the encryption boundary; the share's own ACLs are infra-owned (the engine encrypts but does not manage them) |
...
The engine can serve an **opt-in** (`[api].serve_ui`, default **off**), same-origin, **read-only**
browser ops dashboard under `/ui`. It is a client of the existing API and reuses every server-side PHI
...
- **No caching.** Every `/ui` HTML response and every PHI JSON read (`/messages*`, `/dead-letters`) is
  served `Cache-Control: no-store`, so a browser/proxy never retains a message body on disk.
