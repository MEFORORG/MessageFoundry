...
| `mefor-restore-*` staging dirs | no window on a clean exit — the `TemporaryDirectory` unlinks, and a restore is an operator-driven one-shot rather than a scheduled repeat, so nothing accumulates. It survives a crash or `SIGKILL` like the rows above. Unlike them the engine DOES apply its file ACL here (`_secure_file` on the staged tar and on the extracted store), so the operator's directory ACL is the backstop rather than the only control |
