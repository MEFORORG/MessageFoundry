...
*A companion standard to Secure Development Standards. It governs how the maintainers **use Claude Code to BUILD** MessageFoundry — the engineering workflow — and is NOT about the AI assistant the shipped product offers operators (that is [AI.md](AI.md)). The two share the word "AI" and nothing else.*

> **Scope boundary — read this first.** This standard governs **using Claude Code to BUILD** MessageFoundry and any project developed under the Secure Development Standards — the maintainers' **own engineering workflow**. The shipped **product's** runtime AI-assistant policy (an operator-set `mode` × `data_scope` clamped by production posture, RBAC-gated by `ai:assist`) lives in [AI.md](AI.md) and is **OUT OF SCOPE here**; the two share a name only. PHI data rules: [PHI.md](PHI.md). Identity / RBAC: [SECURITY.md](SECURITY.md). The project's standing contract for an AI session is [`../CLAUDE.md`](../CLAUDE.md). These are **complementary** — read the relevant ones.

**Standard ref:** Secure Development Standards **PO.5** (secure development environments — *no real PHI in dev/test*), **PO.2 / PW.7** (code review & analysis), and **§A.6** (the *Single-maintainer development → AI-assisted review as a compensating control* deviation). This document **owns and expands** that §A.6 deviation as its detailed record for the solo-maintainer reality.

| | |
|---|---|
| **Document** | Secure AI-Assisted Development Standards |
| **Applies to** | Any project developed under the Secure Development Standards using an AI coding assistant. **MessageFoundry (MEFOR)** is the reference implementation (Appendix A); future projects add Appendix B, C, … |
...
This is the *Secure AI-Assisted Development Standards* — a companion to the Secure Development Standards (SDS) that governs **how the maintainers use Claude Code to BUILD MessageFoundry** (the engineering workflow), without weakening any SDS guarantee. It is explicitly **NOT** the runtime AI assistant the shipped product offers operators; that policy lives in [AI.md](AI.md) and is out of scope here (the two share only the word "AI"). The standard **owns and expands** the SDS §A.6 deviation — *single-maintainer development, with AI-assisted review as a compensating control* — as the detailed record for the solo-maintainer reality, and it aligns to **NIST SP 800-218 (SSDF)** and **OWASP ASVS 5.0 L3** while deliberately conferring **no certification**. **Status: Draft for review, v0.4.** It serves maintainers (the daily loop), adopters and auditors (build-provenance evidence for their own compliance program), and future projects (their own appendix).
...
| Secure-development baseline (SSDF / ASVS / HIPAA mapping) | Secure Development Standards |
...
| **Control parity** — symmetric coverage (§6.6) | Apply the control where you add it | + **enumerate sibling paths** when adding/changing a control (other connectors, routes, platforms, CRUD verbs) and cover them or record the gap | + a **deterministic parity check** over all instances where feasible (one exposure-bind guard across every cleartext listener) | + an **asymmetric** security control is a release-gate **finding**, not an accepted default |
| **Provenance / traceability** (§6.7) | None required | **One coherent layer per commit** + PR + the PR tier-declaration. *(The specified **`Co-Authored-By` model/version trailer** is **not in use** — turned off at source, §6.7.)* | + ADR link for hard-to-reverse decisions + the Plan artifact + PR-thread reference (no separate transcript store is retained yet — §9) | + **claims-register entry** + **AI- vs human-authored provenance** (commit-granular trailer; an adopter's QMS would want line-level — *by analogy*, §10) + **SOUP assessment** of AI-generated code *in a 62304-regulated deployment* (§10) |
...
- **WebSearch / WebFetch** are live egress: **no PHI, secret, or customer-identifying string in any query or tool call** (a partner name, internal hostname, or config fragment). The repo's [`scan_forbidden.py`](../scripts/security/scan_forbidden.py) backstops forbidden strings that **land in a commit / the published mirror** — it is **not** a live interceptor of a WebSearch/WebFetch/MCP query, so runtime egress discipline is on the human. **At T3, web egress is off or reviewed.**
...
**Coordinate globally-unique identifiers across isolated sessions.** Isolated worktrees are the right default — but they also mean parallel agents **cannot see each other's new global identifiers** (ADR / migration / backlog numbers). **Reserve the number against a shared registry before use**, and **reconcile against `origin/main`'s latest before merge** — an isolated session that picks "the next free ADR number" from its own checkout *will* collide with a sibling that did the same, and the duplicate surfaces only at merge (different filenames → no git conflict to catch it). *Evidence:* in the 2026-06 audit, ADR `0034` was independently claimed by three concurrent branches; the collision was found post-merge and needed a fix-up PR.
...
> **Scoped-green ≠ the gate.** An agent that runs a **deselected subset** of the suite (only the tests near its change) and reports green has **not** passed the gate. "Done" means the **same gate that governs merge** — the full local gate **and** the blocking CI — including the **cross-cutting invariant gates** (crypto-inventory / SPDX / dep-lock) and **every platform leg**, none of which a change-local test run exercises. *Evidence:* two AI lanes in the 2026-06 audit reported green on their targeted tests yet failed CI — one tripped the crypto-inventory gate (a new `ssl` import in a file not in the inventory), one failed only on the Windows legs.
...
**Control parity is a review gate.** When a diff adds or changes a security control, the reviewer **enumerates the sibling paths** the same control should cover — the other connectors, routes, platforms, and CRUD verbs — and confirms **symmetric coverage** or records the gap as a finding. An AI works in a **narrow slice** and reliably implements the control *where prompted* while missing its siblings: in the 2026-06 audit, **every** confirmed medium-or-higher finding was exactly this shape (TLS verified everywhere but FTPS; a cleartext-bind guard on some listeners but not others; a guard live on POSIX but a no-op on Windows; a last-admin guard on role-removal but not user-disable; per-channel scope on every PHI route but not the topology reads). Where the parity can be encoded as **one deterministic check** over all instances (a single exposure-bind guard across every cleartext listener), prefer that to a recurring manual sweep (§6.5).

**Treat an agent's report of an outward-facing action as a claim, not a fact.** "PR opened", "branch pushed", "merged", "all green" — **verify each against the system itself** (`gh pr view`, the actual CI rollup, the merged ref) before relying on it. An agent's self-report of a side-effecting action is a known confabulation surface; the ground truth is the system state, not the transcript.
...
> `Signed-off-by:` an AI cannot give. Any adoption needs (a) a CLA-compatible trailer verified against
> `cla.yml`'s allowlist, and (b) enforcement — a `commit-msg` hook plus an ungated CI backstop, the
> two-layer shape the ledger gate already uses to survive `--no-verify`. Until both exist, do not
> reintroduce the prescription.

For reference, the format that was specified (**not currently used** — see the box above):
...
The human-gate stack: [CODEOWNERS](../.github/CODEOWNERS) + branch protection + required CI checks + the [PR template](../.github/PULL_REQUEST_TEMPLATE.md) + the CLA bot ([`cla.yml`](../.github/workflows/cla.yml)). For the **tag-time** bar (SBOM / signed tag / SLSA), see RELEASE-GATE.md. Note `CODEOWNERS` is **documentation, not enforcement** here: `require_code_owner_reviews` is false and the sole owner is the sole author.

> **Provenance honesty (read with §9).** With the trailer unused, the durable provenance record is the
> **PR thread + commit history + this standard**, at commit granularity. There is no model/version
> record in the commit stream today, so the project cannot answer "which model wrote this, under what
> policy" for any commit in its history — and there is **no** retained-transcript store either (§9).
> The line/hunk-level distinction a QMS actually wants is **designed-but-deferred**. Do not claim more
> than this delivers (§8; Appendix A.6 deviation). `tests/test_ai_provenance_claims.py` fails if the
> trailer is re-listed as Built or as evidence while the repo still instructs omitting it.
...
- One coherent layer per commit + PR + the PR template's tier/AI declaration. *(The `Co-Authored-By` model/version trailer is **NOT** in this list: the project turns it off at source, via `attribution` in [`.claude/settings.json`](../.claude/settings.json), and the cause once recorded here is withdrawn — see §6.7, A.6, and the deferred list below.)*
...
**Evidence set.** *Retained, auditable evidence:* this standard + [`../CLAUDE.md`](../CLAUDE.md); the [`.claude/settings.json`](../.claude/settings.json) hook/deny-list config; the security-CI scan history; PR/commit history; the **claims register** (A.4); the **deviations register** (A.6). *Ephemeral (not retained today):* the live session transcript — there is **no** retained-transcript store yet (§9), so it is not a standing archive; the PR thread + commit history are the durable provenance until that store is built. **Removed from this set 2026-07-29:** the `Co-Authored-By` model/version trailers. They were listed as retained evidence while the measured count was **zero** (§6.7) — evidence that does not exist cannot be produced on request, and citing it is worse for an audit than declaring the gap.
...
| Customer/PHI leak guard | [`scripts/security/scan_forbidden.py`](../scripts/security/scan_forbidden.py) | Built |
...
| Listener cleartext-bind + source-IP **parity** guard (one check over all LISTEN connectors) | [`messagefoundry/pipeline/wiring_runner.py`](../messagefoundry/pipeline/wiring_runner.py) | Built |
| Config-source trust (NTFS-DACL / POSIX) — fail-closed default + audited dev escape | [`messagefoundry/config/wiring.py`](../messagefoundry/config/wiring.py) | Built |
...
|---|---|
| "Built with AI assistance under the Secure AI-Assisted Development Standards." | This doc; PR/commit history; CI history. *(Previously cited `Co-Authored-By` trailers — struck 2026-07-29: measured count zero, §6.7.)* |
...
| ~~"Provenance recorded at commit granularity by convention."~~ **WITHDRAWN 2026-07-29 — do not use this wording.** | The `Co-Authored-By` trailer this rested on is **not in use** — the project turns it off at source, via `attribution` in [`.claude/settings.json`](../.claude/settings.json) — and the cause once recorded here is withdrawn (A.6). Past adoption is not measurable from a shallow clone (§6.7). The defensible replacement claim is: *"AI involvement is disclosed per PR via the PR template; there is no per-commit model/version record."* |
| "AI-surfaced security findings are independently (multi-vote) verified before action; the residual risk is control-asymmetry, addressed by a parity review gate." | The 2026-06 audit (44 candidates → 26 confirmed); §6.6 parity gate; the parity guard/tests (A.2). |
| "AI-added security controls are validated against the governing CI gate and real environments, not the model's self-selected tests." | §6.5 'scoped-green ≠ the gate'; the crypto-inventory + Windows-leg CI catches. |
...
- **Trailer format:** **none in use.** The project turns the `Co-Authored-By` trailer off at source, via `attribution` in [`.claude/settings.json`](../.claude/settings.json), so the specified `Co-Authored-By` + `Tier:` block is never emitted. The cause once recorded here is withdrawn (A.6), and past adoption is not measurable from a shallow clone — see §6.7 before reinstating it anywhere.
...
**Links out** (this doc → the docs graph): [`../CLAUDE.md`](../CLAUDE.md) (the operationalized standing contract) · Secure Development Standards §A.6, PO.5, PW.7 (bound via the **Standard ref** above) · [AI.md](AI.md) (out-of-scope product policy) · [PHI.md](PHI.md) (PO.5 no-real-PHI + treat-HL7-as-untrusted-data) · [WORKTREES.md](WORKTREES.md) · security/RELEASE-GATE.md (tag-time gate) · [docs/adr/README.md](adr/README.md).

**Reciprocal back-links (in place):** **FROM** SDS §A.6 — the *Single-maintainer development* deviation names **this doc** as its detailed record (SDS v1.5); **FROM** [AI.md](AI.md) — both the *scope* callout (product feature ≠ dev process) and the §4.5 BAA *dev-process analogue* cross-reference this standard. *Not applicable:* a [docs/adr/README.md](adr/README.md) row (no ADR was cut for this standard) and a [FEATURE-MAP.md](FEATURE-MAP.md) entry (it enumerates security *features/controls*, not docs).
