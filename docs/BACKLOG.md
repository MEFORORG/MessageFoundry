...
> 🚧 **Status OPEN (filed 2026-08-01; both named instances now closed, the generic remedy is not).** Value **6/10** · Difficulty **4/10** · _fill-in_. Hardcoded real-time budgets — a CI `step_timeout`, a test helper's poll deadline — were sized when the work was smaller or the machine faster, and nothing re-derives them. Instance 1 (the Windows `step_timeout`) was fixed on filing; **instance 2 was resolved 2026-08-02 and turned out not to be a bound problem at all** — a swallowed SQL Server lock-timeout stranded a lane in a terminal IDLE, and no budget would have helped. What remains open is the mechanical margin check (proposal 1) and the sweep of the remaining bounds (proposal 2). The item's real lesson is now instance 2's: **a timeout with zero failing assertions is a diagnosis to be made, not a number to be raised.**
