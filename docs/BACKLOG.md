...
> 🚧 **Filed 2026-08-01; in progress 2026-08-06 (BACKLOG #332) — sigstore hash-pinned via the release-tools lock; build/cyclonedx-bom deferred as a follow-on.** Value **6/10** · Difficulty **5/10** · _quick win_. Arbitrary code from any of ~30 floating transitives at `.github/workflows/release.yml:255` runs with the OIDC identity that then signs the wheel, writes the SLSA attestation and publishes to PyPI — a backdoored artifact carrying a *valid* Sigstore bundle and valid provenance — and no Dependabot ecosystem parses an inline `pip install X==Y`, so the pin rots with no trigger and no owner (the two siblings at `:104` and `:207`, the latter a `~=` range, float identically); the ADR 0034 hashed-lock mechanism is proven and running for `ci-scanners`/`ci-quality`, but `sigstore` is absent from every lock (`grep -c sigstore uv.lock` → 0), adding a seventh is a six-place lockstep edit, the resolve contamination may force the same excluded-by-decision call semgrep got, and no PR leg ever executes this path.
>
> **AMENDED 2026-08-06 (BACKLOG #332) — PARTIAL.** The `sigstore` half is built: `release.yml`'s signing step now installs `pip install --require-hashes -r ci/locks/release-tools.lock` (a non-default `release-tools` PEP 735 group pinning `sigstore==4.4.0`, exported to a 31-requirement/193-hash closure that rides the DEP-1 export+resync machinery), closing the floating-transitive gap at the pipeline's highest-privilege point. Contamination measured 0 (the four runtime + two CI exports re-export byte-identically) and the closure resolves under `--require-hashes` on CPython 3.14; the version stays `4.4.0` (the recorded ADR 0034 decision), so the body's `grep -c sigstore uv.lock` → 0 line is now **inverted by design** — a non-zero count is the fix, not drift. Still open: the two siblings the body flags — `build` (`:104`) and `cyclonedx-bom` (`:207`) — remain top-only pins; folding them into the group is the reported follow-on (it must move the SBOM install in lockstep across `release.yml` + `security.yml`). Banner left `🚧` pending owner disposition (partial vs full close). Framing: NOT-DEPLOYED beta — an unhashed release toolchain **would** be a supply-chain gap at a first published tag, not a live exposure.
