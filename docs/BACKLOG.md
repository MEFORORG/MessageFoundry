...
**The eight findings.** Written in the conditional throughout — nothing here is live.
...
**Finding 4 is the one to read carefully, and the naive fix is already known to be insufficient.** Measured 2026-08-05 on .NET 10.0.9 / Windows 10.0.26200, 16 racers over 500 rounds, instrumented in C# rather than PowerShell (PowerShell scriptblock-as-delegate closures do not capture loop variables reliably and would have produced a clean-looking result for the wrong reason): `[System.IO.File]::Move(src, dst)` **returns success without moving** for losers under contention. Every one of 500 rounds had more than one racer return with no exception; in **375 of 500** rounds **all sixteen** did. And the obvious remedy — verify `File.Exists(dst) && !File.Exists(src)` after the move — was **true for all 16 racers in all 500 rounds**, because the winner's move makes it true for everybody. With a destination unique per claimer, **exactly one** claimer's own destination existed in **500 of 500**. Controls: a `Move` of a source that never existed, and a sequential `Move` of an already-moved source, both raise `FileNotFoundException`, so the silent no-op is contention-only. **Therefore the primitive is: move to a name no other claimer could mint, then verify your own destination.** Three call sites: the drain's expiry sweep, the drain's inbox-to-seen claim, and the sender's publish from `tmp/` into an inbox.
...
**DO NOT WIRE anything as part of this item.** The installer rows for the drain stay exactly as they are, and nothing is installed into any config root. Wiring places a hook on `SessionStart` and `Stop` for every session in the repo and is a separate owner decision.
