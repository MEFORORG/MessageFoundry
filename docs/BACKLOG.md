...
**Anchors, re-derived on `origin/main` at 6e481c14 now that #326 has merged.** These resolve today; verify them before starting.
...
⚠️ **AMENDED at `aa8bb694`, hours after filing, and it SHARPENS the item.** The verification behind this body was performed at `6e481c14`, where `install-git-hooks.ps1` had no payload parity either — so the original wording said the asymmetry was against `install-gate.ps1` alone. **PR #191 then landed payload parity on `install-git-hooks.ps1`**: SHA256 content hashing (`:100`), an explicit PAYLOAD-parity section (`:124`), IN SYNC / STALE reporting (`:155-157`), and `tests/test_installed_coord_hooks.py` asserting the same property from the pytest side on the same folded-comparison basis. So the correct statement is now stronger and simpler: **`install-selfheal.ps1` is the ONLY installer in the estate with no payload-parity instrument**, and two worked examples to copy exist rather than one. Nothing else in this body is affected — #191 did not touch `install-selfheal.ps1`, whose parameter surface is still `-ConfigDir` plus `-HookPath`, and it added no selfheal reference to either parity test.
