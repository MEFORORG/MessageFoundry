...
| **ECH (Encrypted Client Hello) for outbound SNI — buildable off-stdlib, deliberately not owned** | `docs/SECURITY.md:1740-1756` (filed as `:1692`, drifted); ADR 0139 *Disposition*; ADR 0093 §3, corrected 2026-08-10 | **Not** infeasibility — that ground was **refuted** by a working implementation written in this repository, and is corrected under BACKLOG **#1011**. The real condition is that **no destination publishes an `ECHConfig`**, so an ECH client would conceal nothing | At each release, and the re-confirmation is **executable**: re-run the DoH type-65 probe over the deployment's actual egress allowlist. Retire the acceptance the moment a real destination publishes an `ECHConfig` — that, not a CPython or OpenSSL release, is the trigger |
