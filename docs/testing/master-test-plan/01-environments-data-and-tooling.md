...
| CI security scanners | SAST / SCA / secrets / supply chain | `bandit`, `semgrep`, `gitleaks`, `pip-audit` (+ the DEP-1 lock-sync re-export diff), `npm-audit`, `trivy`, SBOM, `crypto-inventory`, `forbidden-content`; plus `codeql` (python + javascript-typescript), `scorecard`, `zizmor`, `kubeconform` | E2 | Blocking: bandit, pip-audit, gitleaks, semgrep, forbidden-content, zizmor, and **CodeQL** (`CodeQL (python)` + `CodeQL (javascript-typescript)` -- both required on the server; BACKLOG #1384). Advisory: Scorecard. The `security-events: write` fork-token reason was written as a justification SHARED by CodeQL and Scorecard, and only the Scorecard half is unrefuted -- see [`.github/required-contexts.txt`](../../../.github/required-contexts.txt) |
...
| [`codeql.yml`](../../../.github/workflows/codeql.yml) | python + javascript-typescript | push→main, PR, weekly `0 7 * * 1`, dispatch | E2 | **Required** (both contexts) | Taint/data-flow SAST — untrusted HL7/config reaching a sink across function boundaries |
