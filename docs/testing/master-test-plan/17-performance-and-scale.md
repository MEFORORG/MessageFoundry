...
| `tests/test_connscale_smoke.py`; `tests/test_connscale_postgres.py` at `MEFOR_STORE_POOL_SIZE=4` (ci.yml:853) | In-process connection-scale N=12→24 on SQLite (no-loss, **FD monotonicity GATED**; **empty-claim monotonicity MEASURED AND RECORDED, NOT GATED** **[COVERAGE PIN DISCHARGED 2026-08-28, in the change it was written for. The pin, added the same day, said: if the empty-claim assertion is disarmed, skipped or deleted, this row must be edited in the same change. It was disarmed -- `empty_claims_monotonic = false` in both profile copies -- so this row now states what is actually covered. WHAT REMAINS: the ratio is still computed on every run and still lands in `out/connscale/empty_claims.json`, and `test_the_empty_claim_curve_is_measured_but_no_longer_banded` asserts that it was genuinely compared, so a silent stop in collection still fails. WHAT IS GONE: any assertion that the ratio stays inside a band. WHY: measured on 252 pairs from 126 green CI legs, the SLO's premise holds on ubuntu 84 times out of 84 and fails on windows-2022 in 38 percent of PASSING runs, so the 0.75 floor sat inside the Windows distribution rather than between two modes. BACKLOG #1211.]**, executor shim, reload probe); pool-acquire-wait wall on a real server DB with a forced tiny pool |
