...
4. **`security.yml`'s own header contradicts its triggers.** Lines 11-16 state "NO push-to-main
   trigger (dropped for CI cost)"; lines 22-23 define `push: branches: [main]`. The re-add is
   justified at `:19-21`, so the stale block is `:11-16`.
...
| The customer/PHI leak gate's detector floor silently falls behind the token list | `MEFOR_MIN_DETECTORS=names=7,estate=13,site_prefixes=1` exists only as a shell literal (`security.yml:403`). Nothing asserts it still matches the real list, that the spec parses, or that a partially-mangled secret is caught. The real list arrives only from `MEFOR_FORBIDDEN_TOKENS`; `scan-tokens.local.txt` is gitignored (`.gitignore:126`) and never committed, so no checkout can hold it | Real partner/site names and estate identifiers reaching a **public** repository | Partially — `test_scan_tokens_source.py` covers the floor **parser**, nothing covers the **value** | **P0** |
...
| ECH's central claim is unprovable and its shipped code is unbuilt and untested | The engine-side hop (ADR 0139 Increment 1) routes egress to a loopback sidecar but **does not originate ECH**; `tests/test_ech_egress.py` (183) proves routing + fail-closed against a stub only. The **terminating re-originator now ships** as `tools/ech-sidecar/` (Go, `main.go` 312 lines + `go.mod` + `README.md`), yet no workflow has a `setup-go`/`go build` step and `pyproject.toml:21` keeps `tools/` out of wheel and sdist — while ADR 0139's status block still files it under "Deferred". The 2026-07-20 DoH probe found no partner publishing an ECHConfig | Any downstream "supports SNI hiding" statement would be unproven; a Go binary nothing builds, tests or version-pins is on the egress path by configuration; the fail-closed route can break real egress for zero privacy benefit | Honest in the ADR about the *engine* half, stale about the sidecar; invisible in the product | **P2** |
...
| SEC-07 | `docs/SECURITY.md` supply-chain section matches HEAD | Negative/Security | pytest | any | n/a | T | P2 | Extend `tests/test_security_doc_drift.py`: for every job name in `security.yml`, the doc's Supply-chain section must classify it BLOCKING/ADVISORY consistently with the YAML; the strings "advisory for now", "Planned CI additions", and "GHAS on a private repo" must be absent while `codeql.yml` exists and the repo posture is public. Currently red at `docs/SECURITY.md:1753-1775`. |
...
| SEC-71 | ECH disposition — **including the shipped `tools/ech-sidecar/` tree** | Negative/Security | manual | any | n/a | C | P2 | The disposition is bigger than "ADR 0139 Increment 1". **Three artefacts exist and none is owned by a build or a test beyond a stub:** (a) the engine-side routing — `ech_route_from_settings` / `egress_route_from_settings` in `transports/rest.py` (see `:1084`), which does **not** itself originate ECH and is covered against a stub proxy by `tests/test_ech_egress.py` (183); (b) **`tools/ech-sidecar/` — the shipped, TLS-terminating Go re-originator** (`main.go` 312 lines + `go.mod` + `README.md` 90; stdlib-only; resolves the destination's ECHConfigList from a DNS HTTPS RR (type 65) over DoH, sets `EncryptedClientHelloConfigList`, and never sets `InsecureSkipVerify`), described at `rest.py:1084` as "proven to hide the SNI against a real ECH endpoint"; and (c) the operator recipe `samples/ech-sidecar/README.md`. **Nothing builds, tests, lints, version-pins or ships the Go tree** — no `setup-go` / `go build` step exists in any workflow (verified by grep over `.github/workflows/`) and `pyproject.toml:21` `only-include` keeps `tools/` out of both sdist and wheel — while ADR 0139's *Implementation status* block still files the terminating re-originator under "**Deferred** (the real ECH work)", which HEAD contradicts. Pass = a dated owner decision covering **all three**: keep (then the sidecar earns a Go build/test leg, a pinned toolchain and a distribution answer) or retire, with the ASVS 12.1.5 residual recorded as a standing accepted Fail either way; ADR 0139's status block reconciled to HEAD; and `docs/SECURITY.md` carrying no "supports SNI hiding" claim while no partner publishes an ECHConfig (2026-07-20 DoH probe). `tests/test_ech_egress.py` stays as the fail-closed guard regardless. |
| SEC-72 | `security.yml` header matches its own triggers | Negative/Security | pytest | any | n/a | T | P2 | Extend `tests/test_security_workflow_liveness.py`: the workflow's header comment block cannot claim "NO push-to-main trigger" while `on.push.branches` includes `main` (`security.yml:11-16` vs `:22-23`). Also asserts `codeql.yml:16-19` and `scorecard.yml:17-19` do not claim version-tag pinning while every `uses:` carries a 40-char SHA. |
...
- **SEC-39, SEC-54, SEC-59, SEC-69, SEC-70, SEC-71** — owner decisions and signatures, not
  ...
  recorded. **S** each, except SEC-71 (**M** if the ECH sidecar is kept, since keeping it implies a Go
  build/test leg and a distribution answer).
...
11. **What is the disposition for ECH (ADR 0139) given that no partner publishes an ECHConfig, yet the
    terminating re-originator `tools/ech-sidecar/` (Go, 312 lines) is *shipped in the tree* while no
    workflow builds, tests or version-pins it and `pyproject.toml:21` keeps it out of the
    distribution — keep it (and give it a Go build/test leg, a pinned toolchain and a distribution
    answer), or retire the code and mark ASVS 12.1.5 a standing accepted Fail? Either way, who
    reconciles ADR 0139's status block, which still files the sidecar under "Deferred"?**
    *Blocks:* SEC-71, SEC-01's status-vs-code half, and any external statement about SNI hiding.
