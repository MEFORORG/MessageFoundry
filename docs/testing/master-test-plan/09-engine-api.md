...
| ~~`GET /uploads` shows every uploader's filenames~~ **CLOSED (owner-only, ASVS 8.2.2)** | Uploaded files are now owner-scoped: the listing returns only the caller's own files and browse/resend/delete answer 404 for another operator's file, unless the caller holds the explicit `files:access_any` override (Administrator only). Enforced in the handler bodies (so the `/ui` seam inherits it) and pinned by `tests/test_upload_api.py` | Was: operator-supplied original filenames can carry patient/facility identifiers, and the `file_id` the listing handed out is the token the browse/resend/delete routes accept | **Yes** | **P2** |
...
| API-37 | `GET /uploads` visibility rule is explicit: two uploaders, one `files:browse` holder | PHI | pytest | container-CI | SQLite | C | P2 | **SHIPPED** — the owner-ratified rule (Q6, ratified 2026-08-11; ADR 0134 Amendment A) is owner-only plus an `files:access_any` override, keyed on the immutable `uploader_id` (`Identity.user_id`), never the reusable username: the second operator gets an empty listing and 404 on browse/resend/delete; a recreated account holding the departed operator's username gets the same 404s; an Administrator sees and reaches the file; a sidecar with no `uploader_id` is reachable only with the override. Pinned in `tests/test_upload_api.py` |
...
6. **RESOLVED (owner-only) — owner-ratified 2026-08-11.** ~~Is `GET /uploads` intended to show every
   uploader's files to any `files:browse` holder, or should it be owner-scoped like search presets?~~
   Owner-scoped, with an explicit `files:access_any` override granted to Administrator only and never
   mintable onto a custom role. The channel axis was **rejected**: `allowed_channels` defaults to every
   channel, so a channel-scoped rule would protect nobody on a default install — a control resting on a
   false premise. Ownership is keyed on the **immutable `uploader_id` (`Identity.user_id`)**, never the
   reusable `username`. Recorded as a reviewed decision in
   [ADR 0134](../../adr/0134-offline-uploaded-logs-viewer-connection-decoupled-upload-browse-resend-deletion-phi-at-rest-posture-stdlib-multipart.md)
   (Amendment A), which is what exit criterion 12's **second clause** requires. Criterion 12 as a whole
   is **still open**: its first clause (every pageless list route pages or is on `_UNPAGED_EXEMPT` with a
   reason) is untouched, and `GET /uploads` remains pageless. API-37 is unblocked and shipped.
   *Note on the question's own premise:* saved search presets, named here as the owner-scoping
   precedent, are scoped by the **reusable `username`** — the exact key the uploads path deliberately
   abandoned. The precedent carries the defect the answer removed, so it must not be cited as the model
   to copy until it is re-keyed.
