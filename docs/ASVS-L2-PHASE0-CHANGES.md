...
| **Browser WebSocket Origins rejected** | On the **native** (header) path a `/ws/stats` handshake carrying an `Origin` header is rejected unless the Origin is allow-listed (ASVS 4.4.2); a native client sends no Origin and is unaffected. The `/ui` web console is authorized *ahead* of that path by a **same-origin** `Origin` check (against `[security].web_console_public_address` when set, else `Host`) plus its `SameSite=Strict` cookie, so it needs no allow-list entry. | Only a client that sends both an `Origin` and the `Authorization` header needs its Origin listed in `[api].ws_allowed_origins`. A browser cannot send that header on a WebSocket, so listing a browser's Origin does not admit it while authentication is on. |
...
|---|---|---|
| `[api].ws_allowed_origins` | empty | `Origin` values the header-token path accepts on `/ws/stats`. Empty ⇒ only no-Origin (native) clients on that path; the `/ui` console's same-origin cookie path runs ahead of it (see *Browser WebSocket Origins rejected* above). Env: `MEFOR_API_WS_ALLOWED_ORIGINS` (path-sep list). (4.4.2) |
...
The 2026-08-20 research on BACKLOG #1162 and #1163 then found more with no row of their own: the DR backup archive,
the TOTP shared secret, the MFA recovery codes, the anonymizer salt, the CSP nonces, the alert
webhook hop, and the TLS private keys the listeners and client hops load. Those rows are in the table
now too. Read the table as covering **at least** what it lists rather than as closed, and treat a
surface you cannot find here as uninventoried rather than as absent from the engine.
...
| Session tokens | 256-bit CSPRNG (`secrets.token_urlsafe(32)`); stored only as SHA-256 | `sessions.token_hash`. Client side, the VS Code extension keeps the plaintext token in VS Code SecretStorage, keyed by engine URL (`ide/src/auth.ts`) | Idle 30 min / absolute 12 h; revoked on logout/password-change/disable |
...
| Tray → engine TLS (local status probe, [ADR 0113](adr/0113-windows-tray-service-manager-stdlib-ctypes-tokenless.md) 2026-07-22 amendment) | Verifies the engine API server cert on the tray's **tokenless** `/health` + `/ui` probes on the https loopback bind the engine serves by default (ADR 0172): **a pinned PEM or the OS trust store**. The engine's minted `api-generated-cert.pem` is pinned as the only anchor (`ssl.create_default_context(cafile=...)`), found from the service entry or set as `engine_cacert` in `tray.toml`; an operator chain verifies against the OS trust store (`truststore.SSLContext`); `ssl` in `tray/probe.py`. **No `verify=False` escape**, and an unloadable pin falls back to the OS store, never to no verification. An AST test in `tests/test_tray_probe.py` freezes that | The engine's minted certificate beside its store database, or the Windows machine trust store (an internal-CA/AD-CS cert verifies as-is; a self-signed operator cert is installed under Trusted Root) | Managed by the OS trust store; a verification failure renders the engine `DOWN`, never an unverified connection |
...
| Vendored CLA action provenance (BACKLOG #1578) | At least three digests, computed in [`scripts/security/build_cla_action_provenance.py`](../scripts/security/build_cla_action_provenance.py) and recorded in [`.github/actions/cla-assistant-lite/provenance.cdx.json`](../.github/actions/cla-assistant-lite/provenance.cdx.json): SHA-256 over the vendored action's files and over the upstream bundle body; SHA-1 as git's blob id for the vendored lockfile (`usedforsecurity=False`); and SHA-1 inside `uuid.uuid5` for the record's serial number. **A change detector and a provenance derivation, not a security control** -- no secret, no key, no message authentication, and every input is third-party code or a public URL committed beside the digest. SHA-1 appears only where an outside format fixes it: git names a blob by its SHA-1, and a version-5 UUID is defined over SHA-1. Replacing either would give a number nobody else can reproduce. The crypto discovery gate sees the `hashlib` import but not the `uuid5` use, because `uuid` is not one of its triggers | Not a secret: every digest is committed in the record beside the files it pins | Verified by the builder's `check`, which `tests/test_cla_action_provenance.py` runs against the committed tree; a stale value fails that gate, and `--write` regenerates it |
...
| DR backup archive (`.mfbak`, [ADR 0049](adr/0049-turnkey-dr-backup-restore-verify.md)) | Chunked AES-256-GCM in [`store/backup_codec.py`](../messagefoundry/store/backup_codec.py) (`cryptography` `AESGCM`): one fixed-size chunk per frame, a fresh random 96-bit nonce per chunk, and AAD that binds the header's SHA-256, the frame counter and a final-frame flag, so a reordered, dropped, truncated or appended chunk fails to open. The header carries the key's one-way `key_id`, and each frame is charged to the same persisted per-key GCM invocation bound as the store cipher (ASVS 11.3.4). **Usage scope:** there is no separate archive key. The archive is sealed under **the same resolved store DEK** as the at-rest columns above, and it protects the SQLite snapshot plus **every regular file in the config directory**, so a key or secret kept there travels inside it. It is not a transport, signing or token key. A box that resolves **no** store key and sets the audited `[backup].allow_unencrypted` escape writes `.mfbak.plain` instead, which no key protects; a keyed box still writes a sealed `.mfbak` | The archive file at the configured local or UNC destination; the DEK is resolved through the ADR 0019 KeyProvider seam, the same way the store resolves it | Follows the store DEK. After a rotation, older archives stay sealed under the retired key, so keep that key in `MEFOR_STORE_ENCRYPTION_KEYS_RETIRED` for as long as those archives are kept. Discarding the DEK erases every archive sealed under it |
| TOTP shared secret (second factor, WP-14) | RFC 6238 time-based codes over HMAC-SHA-256 (`hmac` + `hashlib` in [`auth/totp.py`](../messagefoundry/auth/totp.py)), keyed on a 160-bit secret from `secrets.token_bytes(20)`. Verification uses `hmac.compare_digest`, and each matched time step is consumed once. **Usage scope:** this key **only** proves that one user holds their authenticator, at login and at step-up. It encrypts nothing, signs nothing, derives no other key and is never shared across accounts. Its lifecycle and its two-holder bound are in the key-management subsection below | `users.totp_secret`, sealed by the store cipher (rows above) and bound to the user id | Minted at each enrolment and cleared by disable or an administrator reset; see the TOTP row in the key-management subsection below |
| MFA recovery codes | Single-use codes from a 31-symbol alphabet with no look-alike characters, six groups of five (about 148.6 bits each), drawn with `secrets.choice` in [`auth/totp.py`](../messagefoundry/auth/totp.py). Stored only as argon2id hashes, and a spent code is deleted in the same step that accepts it | `users.totp_recovery_codes` (argon2id hashes only) | Minted and shown once, at enrolment (`[auth].mfa_recovery_code_count`, default 10); each is deleted when spent; all are cleared with the TOTP secret |
| Anonymizer re-identification salt ([ADR 0030](adr/0030-anonymization-test-harness-tee.md) §4) | Keyed BLAKE2b (`hashlib.blake2b` with the salt as key, 128-bit digest) in [`anon/keying.py`](../messagefoundry/anon/keying.py), vendored byte-identical to `tee/anon/keying.py`. The digest seeds the pick of a surrogate from a fixed pool; the keyed hash, not the pick, is what makes the mapping one-way. **Usage scope:** this salt keys **only** the pseudonymization of one dataset in the tee `anonymize-captures` export. The running engine's message path does not use it, and it protects no data at rest or in transit. It must not be reused across datasets, because a shared salt links their surrogates. Anyone holding it can recompute the surrogate for a guessed real value, so ADR 0030 classes it as PHI-equivalent | Operator-generated and supplied through the environment (`MEFOR_ANON_SALT`, or the variable `--salt-env` names); held only in the export process's memory | Per dataset; see the anonymizer row in the key-management subsection below |
| CSP nonces (web console and VS Code extension webviews) | A fresh CSPRNG nonce per render: `secrets.token_urlsafe(16)` in [`messagefoundry_webconsole/_security.py`](../messagefoundry_webconsole/_security.py) for `/ui` responses, and `randomBytes(18)` from `node:crypto` in `ide/src/cspNonce.ts` for the extension's webviews. The second is first-party crypto outside Python; the crypto gate's non-Python randomness arm is what finds it | Never stored: written into one response's CSP header and its script tags | Single-use, one per response or webview render |
| Alert webhook HTTPS hop (`[alerts].webhook_url`) | Verifying TLS through urllib's own default HTTPS handler, built by `build_asserted_https_handler` in [`config/tls_policy.py`](../messagefoundry/config/tls_policy.py) and asserted forward-secret (ASVS 12.1.2); certificate and hostname checks come from that default context. The opener follows no redirect. An `http://` URL is refused unless the `MEFOR_ALLOW_INSECURE_TLS` escape is set, and that escape is read through the [ADR 0092](adr/0092-posture-keyed-transport-hop-refusal-refuse-the-insecure-phi-hop.md) clamp, so it is inert on an enforcing PHI instance. **Usage scope:** a **trust anchor**, not a key the engine holds. It authenticates the webhook receiver's TLS identity on the alert hop only; it signs nothing, encrypts nothing at rest and has no private half. A path token some webhook services put in the URL is a credential, not a key | The trust store urllib's default context loads; there is no per-hop CA setting | Managed by the operator / OS trust store |
| TLS private keys (the API listener, connections, the log forwarder and the native API client) | Operator-supplied PEM private keys loaded into an `ssl` context by `load_cert_chain`: the API listener ([`api/tls.py`](../messagefoundry/api/tls.py), including the placeholder the engine mints under ADR 0172), MLLP and the inbound HTTP listener (`transports/mllp.py`), DICOM (`transports/dicom.py`), SOAP mutual TLS (`transports/soap.py`), FTPS (`transports/remotefile.py`), the syslog/SIEM forwarder (`logging_setup.py`) and the native API client (`apiclient/client.py`). The key type follows what the site supplies. **Usage scope:** each key **only** authenticates this engine, or for the native client that client, in the TLS handshake of the one listener, connection or hop that names it. It encrypts nothing at rest, signs no message and derives no other key; the traffic keys come from the handshake's key exchange, not from this key | PEM file paths: `[api].tls_key_file`, the connection settings `tls_key_file` / `client_key_file` (passphrase through `env()`), the combined `[logging].forward_tls_client_cert`, or the `EngineClient` `tls_client_key` argument | See the key-management subsection below; rotate before the certificate expires |
...
TLS placeholder key and the TOTP secret made that false. Apart from the load-test harness pair,
every other key below is minted by the deploying site or by a counterparty, and the engine only
loads it. (Short-lived CSPRNG secrets the
...
**Distribution.** For each operator-supplied key in the first table the engine needs **one** holder:
the process account, reading an `env()`-resolved value or a file. The native API client key is the
...
- **What the project commits to, which a reader can check in the code.**
  ...
  - The TOTP secret has two holders by design. The engine shows it once per enrolment, through the
    JSON API or the web console's enrolment page, and refuses to re-enrol while MFA is on.
    `test_the_totp_secret_is_returned_once_and_never_again` in `tests/test_mfa.py` pins the service
    half and the JSON models; it does not scan HTML pages.
  - The store DEK's escrow is the one second holder this project asks for, for the reason the
    Distribution bullet above gives.
  - **The engine makes at least two further copies, and a site can trigger either by accident.**
    `cert import` writes the key out of a `.pfx` as an unencrypted `key.pem` and leaves the `.pfx` in
...
stored under the DEK.

**The API TLS placeholder retires differently.** No setting names it, so nothing stops naming it.
Once an operator chain is configured the engine serves that chain instead, but it deletes nothing:
...
|---|---|---|---|---|---|---|
| MLLP listener (`mllp`) | inbound | TCP; port authored per connection (no default), bound to `[inbound].bind_host` (default `127.0.0.1`), overridable per connection by `bind_address` | opt-in per-connection TLS (`tls`, TLS 1.2+ floor) with opt-in mTLS via `tls_ca_file`; default loopback plaintext; a **non-loopback cleartext listener is refused at start** unless `serve --allow-insecure-bind` — and that flag is **CLAMPED** on a production-PHI instance, where the per-connection `tls_hop_attested` (with its mandatory `tls_hop_attested_reason`) is the only per-hop opt-in | network trust + optional per-connection `source_ip_allowlist` (CIDR peer gate, refused at accept) + HL7 ACK/NAK | bind host is `[inbound].bind_host`; the peer dials in | `MLLP(port, max_connections, receive_timeout, max_frame_bytes, tls*)`, `[inbound].bind_host`, `bind_address`, `tls_hop_attested`, `[inbound].source_ip_allowlist` |
...
| Raw TCP endpoint (`tcp`) | inbound + outbound | TCP with configurable delimiter framing; port per connection (no default); the inbound bind is `[inbound].bind_host`, overridable per connection by `bind_address` | none on this connector — wrap it in a TLS terminator or use MLLP/HTTP if the hop needs TLS. A **non-loopback cleartext listener is refused at start** unless `serve --allow-insecure-bind` (CLAMPED on an enforcing instance; `tls_hop_attested` with its reason is the per-hop opt-in) | network trust + `source_ip_allowlist` (inbound) | **yes** — `host`/`port` per Connection (outbound); bind is `[inbound].bind_host` | `Tcp(host, port, framing, max_connections, receive_timeout, connect_timeout, timeout_seconds)`, `[inbound].bind_host`, `bind_address`, `tls_hop_attested`, `[egress].allowed_tcp` |
| X12 EDI endpoint (`x12`) | inbound + outbound | raw TCP, ISA/IEA-framed (ADR 0012); port per connection (no default); the inbound bind is `[inbound].bind_host`, overridable per connection by `bind_address` | none on this connector; the same **non-loopback cleartext start refusal** applies (`serve --allow-insecure-bind`, CLAMPED on an enforcing instance; `tls_hop_attested` with its reason per hop) | network trust + `source_ip_allowlist` (inbound) | **yes** — `host`/`port` per Connection (outbound); bind is `[inbound].bind_host` | `X12(host, port, max_connections, receive_timeout, max_interchange_bytes, connect_timeout, timeout_seconds)`, `[inbound].bind_host`, `bind_address`, `tls_hop_attested`, `[egress].allowed_tcp` |
