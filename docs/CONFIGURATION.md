...
| `ordering` | enum | `fifo` | default queue ordering per outbound: `fifo` (strict in-order, head-of-line on failure) or `unordered` (batch + rotate-past-failures). `unordered` is read only under `claim_mode="per_lane"` and has no effect under the default `claim_mode="pooled"`, which drains every lane FIFO; it isolates a stuck message and never adds concurrency (a lane sends one message at a time either way). Per-outbound `ordering=` overrides. |
