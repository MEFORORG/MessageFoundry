...
| Key | Type | Default | Notes |
|---|---|---|---|
| `retry_max_attempts` | int | `100` | attempts before a delivery dead-letters. **Finite by default** (BACKLOG #1051): 100 attempts under the backoff below is a 28,215 s / 7 h 50 m 15 s window, long enough to ride out a partner outage without letting a lane wedge indefinitely. Safe as a default because attempts are counted **per row** (an outage burns the cap on roughly the lane heads, not the backlog) and an exhausted row **dead-letters into the replayable DLQ**. Raise or lower it freely; a code-first `retry=RetryPolicy(max_attempts=None)` still means retry forever, for a partner that must never be advanced past (under FIFO that head blocks its lane until it succeeds or is purged). A permanent `AR` reject fails fast regardless. |
