...
| `audit_all_authorization_decisions` | bool | `true` | ePHI access is **always** audited regardless of this switch; this adds full *authorization-decision* tracing on top, so a site can reconstruct what an account actually reached. **On by default since [ADR 0168](adr/0168-default-the-authorization-grant-audit-on-the-console-cannot-flood-it.md)** — it shipped off against a flood risk that was measured afterwards and does not hold (the browser console never traverses the gate that records grants; WebSocket authorization fires once per connection). Turning it **off** is a loosening and is reported as one. "Always audited" is about **coverage**, not about how hard those rows are to alter afterwards: the audit chain is only cryptographically tamper-*evident* on a **keyed** store, and its verify does not catch a truncated tail — see [`[integrity]`](#integrity) |
