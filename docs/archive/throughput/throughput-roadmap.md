...
answered — see [`throughput-build-plan.md`](throughput-build-plan.md) §"2026-07-02 — WS-B RESULT" for
the full block. Headlines: a driver-free commit-storm put the store at **~29,000 commits/s (≥11×
engine demand, log device idle)** and a delayed-durability A/B bought **zero** throughput — **the
store is exonerated; ADR 0063 (no split store) is closed with positive evidence**. The measured wall
is **engine-side per-message CPU** (~193 msg/s intake per engine on an 8-vCPU-class box; two engines =
383, near-linear; engines at 65–81% CPU during collapse, mechanism not yet decomposed). Scale =
**one store + N engine boxes (ADR 0037) + per-message-CPU cuts**, gated on: the **1500-shape test**
(with ≥5–6 sink processes — every prior *delivered* number was throttled by a ~100–140 msg/s
per-sink-process ceiling), the **CPU decomposition**, and **ownership-scoped startup recovery** before
any N-active engines share one store. Also: the hi-N "≥~48 interfaces/engine wall" is **withdrawn
pending re-measure** (single-driver ~457 msg/s ACK-ceiling confound; ADR 0062's inverted-U unaffected).
Open-path costs fixed: **ADR 0064** schema-init fast-path + index-seekable recovery (#703); harness
instruments #698/#711.
...
|---|---|---|---|---|
| **1** | **Collapse the commit depth** — extend the existing combined `handoff` (ingress→outbound in one txn, `sqlserver.py:1259`, no-transform path) to fuse the producer handoffs where no off-loop work intervenes, and cut the standalone `claim_next_fifo` round-trips | **PRIMARY.** Cuts ~7 → ~3–5 RTs/msg ⇒ raises single-lane (~50 → ~70–115) *and* every lane | ⛔ **CLOSED** (was "build first") — built as ADR 0057, measured **−0.56%**; ADR 0107 | Reliability-core: each fused step stays a single committed txn; preserve at-least-once, ACK-on-receipt (ingress durable before ACK), the `attempts+1` poison-guard, per-lane FIFO, finalizer sole authority. ADR + adversarial-invariant tests. |
