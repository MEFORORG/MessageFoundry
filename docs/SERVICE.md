...
(so the running/stopped split in `/status`' KPIs counts them as not running). A lane that failed
to build, was parked by the DR run-profile, or is `deployed = false` keeps showing `failed` /
`filtered` / `not_deployed` instead: those are facts about that one connection, and the halt
already has its own alert.
