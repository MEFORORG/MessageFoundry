...
| **15.2.5** | `partial` | Rule 5, **not** rule 3. ⚠️ **THIS EXAMPLE'S PREMISE EXPIRED 2026-09-04 (BACKLOG #1278): `[sandbox].mode` no longer ships `off`, it ships `subprocess`.** It stood here as the table's worked example of *"a working control that ships off"*, and that sentence is now false of this cell. The **fact is corrected here; the `partial` verdict is left exactly as it stands**, because re-scoring a cell is the tracking seat's act against the vault record and not a Builder's — and because rule 5 may well still hold for a different reason (the confinement is address-space only; the child still reaches `os`/`subprocess` and the host, which is what [ADR 0147](adr/0147-hardened-runtime-isolation-for-router-handler-code-ipc-brokered-sandbox-extends-adr-0087.md) exists to close). **No live worked example of "a working control that ships off" remains in this table.** That is said plainly rather than patched with a substitute, for the same reason 3.7.3 gives below: a stale example teaches the wrong answer, and a missing one only teaches nothing. |
