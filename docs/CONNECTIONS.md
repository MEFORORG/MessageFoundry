...
1. **The event loop's default `ThreadPoolExecutor`**, bound to CPython's `min(32, os.cpu_count() + 4)`
   ...
   - **Inbound strict validation** — the listener runs `hl7apy` strict validate off-loop via
     ...
     defined in `pipeline/ingress_guards.py` and re-exported by `wiring_runner`). An operator resend
     into a strict inbound (BACKLOG #1911) validates on a daemon thread of its own under the same
     timeout. The timeout frees the *listener* but cannot kill the worker — an
