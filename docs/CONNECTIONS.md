...
verify-off egress hop and a weakened database TLS hop.
...
with its reason, and `GET /security/posture` names them in a `tls_hop_attested` loosening. Each
suppressed refusal also logs a WARNING with the reason. The risk entry is in
