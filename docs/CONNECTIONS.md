...
| `harness/mllp.py` (`MllpReceiver`, the Receive tab), `harness/load/sink.py` (`CorrelationSink`) and `harness/reconcile/capture.py` (`CaptureSink`): the test harness's MLLP receivers. The harness is a separate distribution attached to each release, and these take frames from another party, the engine under test, so they count by Manager decision | anything framed. The Receive tab shows it, the load sink times it, and the capture sink appends it to a JSON-lines file | not applicable; a framed stream | `DEFAULT_MAX_FRAME_BYTES` = 16 MiB, the engine's MLLP default. The two sinks take `max_frame_bytes` to raise it, and `python -m harness.reconcile capture --max-frame-bytes` passes it on. An over-cap frame drops its connection with no ACK, and none of it is shown or kept; the Receive tab and the capture sink count each one. The harness's senders read the engine's ACK replies with no frame cap | no unpacking on intake |
