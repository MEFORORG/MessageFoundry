...
| `max_frame_seconds` | in | `60.0` | close a client whose frame takes longer than this from its **start byte to its end byte** (BACKLOG #1725). This is the bound on a peer that trickles one byte at a time: it is never idle, so `receive_timeout` never fires on it, and without this it would hold a slot and up to `max_frame_bytes` of reassembly buffer indefinitely. Runs **beside** `receive_timeout`, not instead of it — a peer that opens a socket and says nothing never opens a frame, so only the idle bound covers that. The connection closes with a `closed` event whose reason reads `frame_deadline`. `None`/`0` = no deadline. |
