...
**There is no review gate.** `review-gate.yml`, `unread-signal.yml` and the required context
`a reviewer has read this` were removed on 2026-09-05 at the owner's instruction. Branch
protection had already dropped that context at some earlier, unrecorded point: read from the API
that day it carried **13** contexts while this repository still claimed 14, so the removal made
the repository match the server rather than changing what the server enforces.

`required_approving_review_count` is 0 and stays 0 — every session pushes as one GitHub identity
and GitHub refuses self-approval, so raising it wedges every PR instead of reviewing any. With no
gate and no approvals, **nothing reports that a green PR was never read**. Review here is a human
practice, not a machine control, and no prose in this repository may describe it as one.
