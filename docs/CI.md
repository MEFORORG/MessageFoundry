...
| `zizmor.yml` | Lints the workflow files themselves for insecure patterns (template injection, over-broad tokens), and runs `actionlint` on the workflow syntax. Hard-fails, but **not a required check** — it is paths-filtered (see `zizmor.yml`), so it does not report on a PR that touches no workflow, and requiring it would wedge every such PR. The `actionlint` pre-commit hook is the local half. |
...
- **`actionlint` runs on every workflow edit — let it.** GitHub interpolates `${{ }}` expressions
  *anywhere* in a `run:` script — comments included — before the shell sees it, so a stray/invalid
  ...
  `zizmor.yml` (which is already paths-filtered (see `zizmor.yml`)). The hook is the load-bearing half —
