...
| `net-helper.yml` | Publishes `mefor-net-helper.exe` (`packaging/net-helper/`), the privileged helper [ADR 0056](adr/0056-engine-managed-vip-failover.md) specifies, and signs it on `main` when the signing secret is set; without the secret the build still passes, unsigned. The workflow's header records the signing policy. **Not a required check**: it is paths-filtered to the helper and its own workflow file, so it does not report on most PRs. |
