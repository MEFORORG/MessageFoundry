...
1. **The prover prints no total.** ``_run_prove_absences`` reports ``proved`` / ``static-screened`` /
   ``skipped`` / ``problem(s)`` and nothing else. Those four do **not** close to the claim count: a
   static-screened claim may *also* raise a SUSPECT problem, and four other outcomes raise a problem
   while incrementing no counter. So the number a reader most wants -- *how many claims did you look
   at* -- is not derivable from that line, and a prover that silently stopped iterating prints the
   same shape as one that iterated over everything.
...
def reconcile(c: Census, r: ProverResult) -> list[str]:
    ...
    Every claim lands in exactly one of {skipped, proved, static-screened, hard problem}, so the
    residual ``claims - (proved + screened + skipped)`` is the count of claims that raised a problem
    without incrementing a counter. Anything that makes that residual negative, or larger than the
    reported problem count, means the two sides are not describing the same population -- which is
    the only way to notice a prover that stopped iterating, since its own summary looks identical.
    """
    bad: list[str] = []
