...
INVENTORY: dict[str, frozenset[str]] = {
    ...
    "messagefoundry/store/keyprovider_vault.py": frozenset(
        {"hvac", "messagefoundry.config.tls_policy", "messagefoundry.store.keyprovider"}
    ),
    ...
    # #1180 (ASVS 12.3.4), NOT one of #1323's eleven above: the FHIR hop reached the seam only
    # when the HTTP egress family gained an expressible trust anchor. Same shared reason as that
    # block -- it resolves a TLS posture through the common helper rather than building a context.
    "messagefoundry/transports/fhir.py": frozenset({"messagefoundry.config.tls_policy"}),
}
