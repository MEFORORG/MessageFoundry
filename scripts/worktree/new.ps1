...
#
# The install lives in ensure-venv.ps1 so a HARNESS-created worktree can reach it too. See that
# script's header for why, and for the two tests that pin its install line. Calling it keeps ONE
# declaration; a copy in each script is the drift those tests exist to catch.
& (Join-Path $PSScriptRoot 'ensure-venv.ps1') -Path $WorktreePath -Python $Python -Sqlserver:$Sqlserver
